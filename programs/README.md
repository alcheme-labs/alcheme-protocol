...
`programs/` contains the core Anchor programs that form the on-chain authority
layer for Alcheme. These programs are supported by `shared/` for common protocol
vocabulary and `cpi-interfaces/` for cross-program cooperation.
...
| Which events are projected by an operator indexer? | Compare emitted events with the public event vocabulary and operator projection contract when available. |
