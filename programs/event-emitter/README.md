...
`event-emitter` is the on-chain event stream program used by core programs to record protocol events that can later be ingested by an operator indexer.
...
flowchart LR
    ...
    chain --> indexer["operator indexer"]
```
...
| Which emitted events are parsed by an operator indexer? | Compare event payload names with the public event vocabulary and operator projection contract when available. |
| Which subscription features are product-facing today? | Check SDK callers and the private runtime/product tree when available. |
