...
import { mapAvatarUploadError } from '@/lib/profile/avatarUploadError';
...
import { formatRelationshipLabel } from '@/lib/knowledge/relationshipLabels';
...
function CrystalThumbnail({ crystal }: { crystal: MyKnowledgeItem }) {
    const params = useMemo(() => {
        ...
        const input: CrystalDataInput = {
            ...
            heatScore: crystal.stats.heatScore ?? 0,
        };
        return computeCrystalVisualParams(input, crystal.crystalParams ?? null);
    }, [crystal.knowledgeId, crystal.circle?.name, crystal.stats.qualityScore, crystal.contributorsCount, crystal.version, crystal.stats.citationCount, crystal.createdAt, crystal.stats.heatScore, crystal.crystalParams]);
...
export default function ProfilePage() {
    ...
    const { requestWalletConnection, requestWalletDisconnect } = useWalletAction();
    ...
    useEffect(() => {
        if (connected && identityState === 'admission_required') {
            router.replace('/connect');
        }
    }, [connected, identityState, router]);
    ...
    const handleSaveProfile = useCallback(async (profileData: { displayName: string; bio: string; avatarFile: File | null }) => {
        if (isE2EMockMode) {
            await updateUser({
                variables: {
                    input: {
        ...
        if (profileData.avatarFile) {
            try {
                const uploaded = await uploadProfileAvatar(profileData.avatarFile);
                avatarUri = uploaded.avatarUri;
            } catch (error) {
                throw new Error(mapAvatarUploadError(error, {
                    mimeUnsupported: t('errors.avatarMimeUnsupported'),
                    tooLarge: t('errors.avatarTooLarge'),
                    genericFailure: t('errors.avatarUploadFailed'),
                }));
            }
        }
        ...
        try {
            const signature = await runWalletAction({
                kind: 'profile_update',
                source: 'profile_edit_modal',
                key: `profile_update:${user.handle}`,
                run: () => sdk.identity.updateIdentity(user.handle, {
                    displayName: profileData.displayName,
                    bio: profileData.bio,
                    ...(avatarUri ? { avatarUri } : {}),
                }),
            ...
            await waitForSignatureSlot(sdk.connection, signature);
            const waitResult = await waitForIndexedIdentity({
                handle: user.handle,
            ...
            throw new Error(normalizeProfileUpdateError(error, {
                ...
                reauthorizationRequired: t('errors.reauthorizationRequired'),
    ...
    }, [apolloClient, isE2EMockMode, publicKey, refreshIdentityState, runWalletAction, sdk, t, updateUser, user?.handle]);
    ...
    return (
        <div className={styles.page}>
                ...
                >
                    ...
                    <div className={styles.totem}>
                        <ProfileAvatar
                            ...
                            className={styles.totemPhoto}
                            fallback={<Hexagon size={64} strokeWidth={1} className={styles.totemIcon} />}
                        />
                    ...
                    {connected && shortAddress ? (
                        ...
                        <button
                            ...
                            onClick={() => requestWalletConnection({ source: 'profile_header' })}
                        >
                            <Wallet size={14} />
                    ...
                    <div className={styles.headerActions}>
                        ...
                        {connected && (
                            <Button
                                ...
                                size="sm"
                                onClick={() => {
                                    void requestWalletDisconnect({ source: 'profile_header' });
                                }}
                            >
                                <LogOut size={14} />
                                            ...
                                            <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
                                                ...
                                                <div style={{ flex: 1, minWidth: 0 }}>
                                                    ...
                                                    <p className={styles.crystalMeta}>
                                                        ...
                                                        {' · '}
                                                        {formatRelationshipLabel(crystal.relationshipAssignment)}
                                                    </p>
                                                </div>
                                            </div>
                                        </Card>
            ...
            <EditProfileModal
                ...
                initialData={{ displayName, bio, handle, avatarUri: user?.avatarUri }}
