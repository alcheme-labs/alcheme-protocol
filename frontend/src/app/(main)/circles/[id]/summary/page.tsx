...
export default function CircleSummaryPage() {
    ...
    const returnCircleId = useMemo(() => parsePositiveInt(searchParams.get('returnCircleId')), [searchParams]);

    const [snapshot, setSnapshot] = useState<CircleSummarySnapshot | null>(null);
    const [topology, setTopology] = useState<CircleSummaryTopologyPayload | null>(null);
    const [cognitiveMapAi, setCognitiveMapAi] = useState<CircleCognitiveMapAiView | null>(null);
    ...
    useEffect(() => {
        ...
        void fetchCircleSummarySnapshot({ circleId })
            .then((result) => {
                ...
                setCognitiveMapAi(result.cognitiveMapAi);
            })
            .catch((error) => {
                if (cancelled) return;
                setSnapshot(null);
                setTopology(null);
                setCognitiveMapAi(null);
    ...
    return (
        <CircleSummaryScaffold
            circleId={circleId}
            backCircleId={returnCircleId ?? circleId}
            snapshot={snapshot}
            topology={topology}
            cognitiveMapAi={cognitiveMapAi}
