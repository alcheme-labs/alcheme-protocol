...
import ComposeDraftEditor from '@/components/compose/ComposeDraftEditor';
import {
    readDraftPostId,
    rememberDraftPostId,
} from '@/lib/compose/composeDraftSession';
import {
    isDraftIntentAllowed,
    isFeedIntentAllowed,
    normalizeRequestedPublishIntent,
    resolveAllowedPublishIntent,
    resolveFeedVisibility,
    type PublishIntent,
} from '@/lib/compose/publishIntent';
...
interface CircleTarget {
    ...
    circleType: GQLCircle['circleType'];
...
function ComposePageInner() {
    ...
    const creatingDraftRef = useRef<Set<number>>(new Set());
    const [content, setContent] = useState('');
    const [selectedCircle, setSelectedCircle] = useState<number | null>(null);
    const [publishIntent, setPublishIntent] = useState<PublishIntent>('feed');
    const [draftPostIdByCircleId, setDraftPostIdByCircleId] = useState<Record<number, number>>({});
    const draftPostIdByCircleIdRef = useRef(draftPostIdByCircleId);
    draftPostIdByCircleIdRef.current = draftPostIdByCircleId;
    ...
    const circles = (circlesData?.myCircles ?? []).filter((circle) => circle.lifecycleStatus === 'Active');
    ...
    const { createContent, loading: feedSubmitting, error: feedSubmitError } = useCreateContent();
    const [draftSubmitting, setDraftSubmitting] = useState(false);
    const [draftSubmitError, setDraftSubmitError] = useState<string | null>(null);
    const isSubmitting = feedSubmitting || draftSubmitting;
    ...
    const submitError = effectiveIntent === 'draft' ? draftSubmitError : feedSubmitError;
    ...
    const activeDraftPostId = selectedCircle !== null
        ? readDraftPostId(draftPostIdByCircleId, selectedCircle)
        : null;
    const showDraftEditor = effectiveIntent === 'draft' && selectedCircle !== null && canPublishDraft;
    const canFinishDraft = Boolean(activeDraftPostId);
    const canSubmitFeed = content.trim().length >= 2 && selectedCircle !== null && hasCircleChoices;
    const headerActionEnabled = effectiveIntent === 'draft' ? canFinishDraft : canSubmitFeed;
    ...
    const ensureDraftForCircle = useCallback(async (circleId: number) => {
        if (readDraftPostId(draftPostIdByCircleIdRef.current, circleId) !== null) {
            return;
        }
        if (creatingDraftRef.current.has(circleId)) {
            return;
        }

        creatingDraftRef.current.add(circleId);
        ...
        try {
            ...
            const draftPostId = created.result.draftPostId;
            if (!draftPostId) {
                setDraftSubmitError(t('errors.createDraftFailed'));
                return;
            }
            setDraftPostIdByCircleId((prev) => rememberDraftPostId(prev, circleId, draftPostId));
        } catch (err) {
            ...
            creatingDraftRef.current.delete(circleId);
            if (creatingDraftRef.current.size === 0) {
    ...
    useEffect(() => {
        if (effectiveIntent !== 'draft' || selectedCircle === null || !canPublishDraft) {
            return;
        }
        void ensureDraftForCircle(selectedCircle);
    }, [canPublishDraft, effectiveIntent, ensureDraftForCircle, selectedCircle]);
    ...
    const handleDone = useCallback(() => {
        if (selectedCircle === null || !canFinishDraft) return;
        router.push(`/circles/${selectedCircle}?tab=crucible`);
    }, [canFinishDraft, router, selectedCircle]);

    const handleSubmit = useCallback(async () => {
        if (!canSubmitFeed || isSubmitting || submitInFlightRef.current || selectedCircle === null || !selectedCircleInfo) return;
        if (effectiveIntent === 'draft') return;
        ...
        try {
            const tx = await createContent({
                ...
                visibility: resolveFeedVisibility(selectedCircleInfo.circleType),
            });
            ...
            if (tx) {
                ...
                setSubmitted(true);
                setTimeout(() => {
                    router.push(`/circles/${selectedCircle}?tab=feed`);
                }, 1200);
            }
        } catch (err) {
    ...
    }, [canSubmitFeed, isSubmitting, selectedCircle, selectedCircleInfo, content, createContent, effectiveIntent, router, t]);
        ...
        <div className={styles.page}>
            <div className="content-container">
                ...
                >
                    ...
                    <motion.button
                        className={`${styles.submitBtn} ${headerActionEnabled ? styles.submitActive : ''}`}
                        onClick={effectiveIntent === 'draft' ? handleDone : handleSubmit}
                        disabled={effectiveIntent === 'draft' ? !canFinishDraft : (!canSubmitFeed || isSubmitting)}
                        whileTap={headerActionEnabled ? { y: 1 } : {}}
                    >
                        {effectiveIntent === 'feed' && isSubmitting ? (
                            <motion.span
                                animate={{ rotate: 360 }}
                ...
                >
                    {showDraftEditor && activeDraftPostId !== null && selectedCircle !== null ? (
                        <ComposeDraftEditor
                            key={activeDraftPostId}
                            draftPostId={activeDraftPostId}
                            circleId={selectedCircle}
                        />
                    ...
                    <div className={styles.visibilityToggle}>
                        ...
                        <button
                            ...
                            onClick={() => {
                                ...
                                if (selectedCircle !== null && canPublishDraft) {
                                    void ensureDraftForCircle(selectedCircle);
                                }
                            }}
