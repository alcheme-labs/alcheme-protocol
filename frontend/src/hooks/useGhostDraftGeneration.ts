...
import {
    ...
    requestGhostDraftGenerationOnPrivateSidecar,
} from '@/lib/api/ghostDrafts';
import { ACCEPT_GHOST_DRAFT } from '@/lib/apollo/queries';
...
export function useGhostDraftGeneration(options: UseGhostDraftGenerationOptions) {
    ...
    const generateGhostDraft = useCallback(async () => {
        ...
        try {
            ...
            const result = await requestGhostDraftGenerationOnPrivateSidecar(input);
