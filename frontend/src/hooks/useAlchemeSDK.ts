...
export function useAlchemeSDK(): Alcheme | null {
    ...
    const sdk = useMemo(() => {
        ...
        const alcheme = new Alcheme({
            connection,
            wallet: anchorWallet,
            programIds: PROGRAM_IDS,
        });
        // Next.js only inlines statically referenced NEXT_PUBLIC_* variables.
        // Inject the frontend's canonical public origin explicitly so SDK
        // relation-proof lookups never fall back to browser localhost.
        alcheme.content.setQueryApiBaseUrl(getQueryApiBaseUrl());
        return alcheme;
