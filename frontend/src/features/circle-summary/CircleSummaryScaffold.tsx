...
import {
    buildCircleCognitiveMapViewModel,
    ...
    type CircleSummaryTopologyPayload,
...
import CognitiveEvolutionRoute from './CognitiveEvolutionRoute';
...
export function CircleSummaryScaffold({
    ...
    const resolvedBackCircleId = backCircleId && Number.isFinite(backCircleId) && backCircleId > 0
        ? backCircleId
        : circleId;
    const backHref = `/circles/${resolvedBackCircleId}`;
    ...
    const cognitiveMap = useMemo(() => buildCircleCognitiveMapViewModel({
        circleId,
        snapshot,
        topology,
        draft,
        ...
        locale,
        cognitiveMapAi,
    }), [circleId, snapshot, topology, draft, outputs, summaryCopy, locale, cognitiveMapAi]);
    ...
    const defaultFocusedMapNodeId = cognitiveMap.topology.nodes.find((node) => node.relationToCurrent === 'current')?.id
        ?? cognitiveMap.topology.nodes[0]?.id
        ?? null;
    const [reduceMotion, setReduceMotion] = useState(false);
    const [focusedMapNodeId, setFocusedMapNodeId] = useState<string | null>(defaultFocusedMapNodeId);
    const [focusedBranchId, setFocusedBranchId] = useState<string | null>(summaryMap?.defaultFocusBranchId ?? null);

    useEffect(() => {
        setFocusedMapNodeId(defaultFocusedMapNodeId);
    }, [defaultFocusedMapNodeId]);
        ...
        return (
            <main className={styles.page}>
                ...
                <section className={styles.outputsSection}>
                    <CognitiveMapPanel
                        map={cognitiveMap}
                        focusedNodeId={focusedMapNodeId}
                        onFocusNode={setFocusedMapNodeId}
                    />
                </section>

                <CognitiveEvolutionRoute
                    map={cognitiveMap}
                    steps={cognitiveMap.evolution}
    ...
    return (
        <main className={styles.page}>
            ...
            <section id="primary-routes" className={styles.outputsSection}>
                <div className={styles.sectionHead}>
            ...
            <section className={styles.lowerGrid}>
                <article id="open-questions" className={styles.card}>
                    <div className={styles.sectionHead}>
