...
export interface ForkContextCopy {
    stateLabels: Record<string, string>;
    restrictionLabels: Record<string, string>;
    referenceTypeLabels: Record<string, string>;
}
...
export function createForkContextCopy(
    t: ForkReadinessTranslator,
): ForkContextCopy {
    return {
        stateLabels: {
            public_source: t('states.public_source'),
            source_gated: t('states.source_gated'),
            sealed_source: t('states.sealed_source'),
            released_summary: t('states.released_summary'),
        },
        restrictionLabels: {
            none: t('restrictions.none'),
            source_gate_required: t('restrictions.source_gate_required'),
            sealed: t('restrictions.sealed'),
            released_safe_summary: t('restrictions.released_safe_summary'),
            revoked: t('restrictions.revoked'),
        },
        referenceTypeLabels: {
            lineage: t('referenceTypes.lineage'),
            source_material: t('referenceTypes.source_material'),
            circle_summary: t('referenceTypes.circle_summary'),
            knowledge: t('referenceTypes.knowledge'),
            sealed_metadata: t('referenceTypes.sealed_metadata'),
        },
    };
}

export function formatForkContextStateLabel(
    value: string,
    copy: ForkContextCopy,
): string {
    return copy.stateLabels[value] ?? value;
}

export function formatForkContextRestrictionLabel(
    value: string,
    copy: ForkContextCopy,
): string {
    return copy.restrictionLabels[value] ?? value;
}

export function formatForkContextReferenceTypeLabel(
    value: string,
    copy: ForkContextCopy,
): string {
    return copy.referenceTypeLabels[value] ?? value;
}
