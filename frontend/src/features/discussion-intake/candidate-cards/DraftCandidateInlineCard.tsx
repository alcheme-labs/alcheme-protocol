...
export default function DraftCandidateInlineCard({
    ...
    const hasGovernanceStatus = Boolean(
        notice.governanceDecisionStatus || notice.governanceExecutionStatus,
    );
    const hasFailureDetails = notice.state === 'generation_failed'
        && Boolean(notice.lastExecutionError || notice.failureRecovery);
    const hasCollapsibleDetails = Boolean(notice.summary)
        || showPendingHint
        || hasMeta
        || hasGovernanceStatus
        || hasFailureDetails;
        ...
        <div className={embedded ? styles.cardEmbeddedRoot : styles.card}>
            ...
            {hasCollapsibleDetails && detailsExpanded && (
                <div className={styles.detailsBlock}>
                    ...
                    {hasGovernanceStatus && (
                        <div className={styles.metaRow}>
                            {notice.governanceDecisionStatus && (
                                <span className={styles.chip}>
                                    {t('meta.decisionStatus', {status: notice.governanceDecisionStatus})}
                                </span>
                            )}
                            {notice.governanceExecutionStatus && (
                                <span className={styles.chip}>
                                    {t('meta.executionStatus', {status: notice.governanceExecutionStatus})}
                                </span>
                            )}
                        </div>
                    )}
