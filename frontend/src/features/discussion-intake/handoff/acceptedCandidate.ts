...
export interface DraftCandidateInlineNotice {
    ...
    governanceDecisionStatus: string | null;
    governanceExecutionStatus: string | null;
}
...
export function parseDraftCandidateInlineNotice(input: {
    ...
    const lastProposalId = normalizeNonEmptyString(input.metadata.lastRequestId)
        ?? normalizeNonEmptyString(input.metadata.lastProposalId);
    ...
    const governanceProposalStatus = normalizeNonEmptyString(input.metadata.governanceRequestState)
        ?? normalizeNonEmptyString(input.metadata.governanceProposalStatus);
    const governanceDecisionStatus = normalizeNonEmptyString(input.metadata.governanceDecisionStatus);
    const governanceExecutionStatus = normalizeNonEmptyString(input.metadata.governanceExecutionStatus);
