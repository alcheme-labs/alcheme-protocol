import {
    ...
    type KnowledgeRelationshipAssignmentView,
} from '@/lib/knowledge/relationshipLabels';
import type { GQLKnowledgeRelationshipAssignment } from '@/lib/apollo/types';
...
export interface CrystalOutputKnowledgeInput {
    ...
    title: string;
    version: number;
    relationshipAssignment?: GQLKnowledgeRelationshipAssignment | KnowledgeRelationshipAssignmentView | null;
...
export interface CrystalOutputViewModel {
    knowledgeId: string;
    title: string;
    /** @deprecated Use sourceDraftVersionLabel. Kept for circle-summary compatibility only. */
    versionLabel: string;
    ...
    internalRecordVersionLabel: string;
    citationCount: number;
...
export function buildCrystalOutputViewModel(
    ...
    return {
        ...
        title: input.title,
        relationshipLabel: formatRelationshipLabel(input.relationshipAssignment),
        relationshipLabelKey: input.relationshipAssignment?.labelKey ?? input.relationshipAssignment?.label?.key ?? 'pending',
        sourceDraftVersion: null,
        sourceDraftVersionLabel: null,
        versionLabel: 'Source draft pending',
        internalRecordVersion: Math.max(1, Number(input.version || 1)),
        internalRecordVersionLabel: formatInternalRecordVersionLabel(Math.max(1, Number(input.version || 1))) ?? '',
...
export function buildCrystalOutputViewModelFromRecord(input: {
    ...
    const sourceDraftVersion = normalizePositiveInt(output?.sourceDraftVersion);
    ...
    return {
        ...
        relationshipLabel: formatRelationshipLabel(input.knowledge.relationshipAssignment),
        relationshipLabelKey: input.knowledge.relationshipAssignment?.labelKey ?? input.knowledge.relationshipAssignment?.label?.key ?? 'pending',
        sourceDraftVersion,
        sourceDraftVersionLabel: formatSourceDraftVersionLabel(sourceDraftVersion),
        versionLabel: formatSourceDraftVersionLabel(sourceDraftVersion) ?? 'Source draft pending',
        internalRecordVersion: Math.max(1, Number(input.knowledge.version || 1)),
        internalRecordVersionLabel: formatInternalRecordVersionLabel(Math.max(1, Number(input.knowledge.version || 1))) ?? '',
