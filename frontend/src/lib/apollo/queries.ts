...
export const POST_FIELDS = gql`
  fragment PostFields on Post {
    ...
    status
    visibility
    rankingAdjustmentState
    rankingAdjustmentFactorBps
    rankingAdjustmentExpiresAt
    liked
    repostOfAddress
    repostOf {
      id
...
export const KNOWLEDGE_RELATIONSHIP_LABEL_FIELDS = gql`
  fragment KnowledgeRelationshipLabelFields on KnowledgeRelationshipLabel {
    key
    displayName
    description
    useCases
...
export const KNOWLEDGE_RELATIONSHIP_ASSIGNMENT_FIELDS = gql`
  ${KNOWLEDGE_RELATIONSHIP_LABEL_FIELDS}
  fragment KnowledgeRelationshipAssignmentFields on KnowledgeRelationshipAssignment {
    knowledgeId
    labelKey
    sourceKnowledgeIds
    sourceDraftId
    ...
    updatedAt
    label {
      ...KnowledgeRelationshipLabelFields
    }
  }
`;

export const CRYSTALLIZATION_OUTPUT_SUMMARY_FIELDS = gql`
  fragment CrystallizationOutputSummaryFields on CrystallizationOutputSummary {
    sourceDraftPostId
    sourceDraftVersion
    sourceDraftVersionLabel
    sourceAnchorId
    sourceSummaryHash
...
export const REVIEW_DRAFT_ISSUE = gql`
  mutation ReviewDraftIssue($input: IssueReviewAssistInput!) {
    reviewDraftIssue(input: $input) {
      draftPostId
      threadId
      threadState
      targetType
...
export const GENERATE_ACCEPTED_ISSUE_REVISION = gql`
  mutation GenerateAcceptedIssueRevision($input: GenerateAcceptedIssueRevisionInput!) {
    generateAcceptedIssueRevision(input: $input) {
      jobId
      status
      postId
      autoApplyRequested
...
export const APPLY_ACCEPTED_ISSUE_REVISION = gql`
  mutation ApplyAcceptedIssueRevision($input: ApplyAcceptedIssueRevisionInput!) {
    applyAcceptedIssueRevision(input: $input) {
      generation {
        generationId
        postId
        draftText
...
export const GET_KNOWLEDGE = gql`
  ${KNOWLEDGE_RELATIONSHIP_ASSIGNMENT_FIELDS}
  ${CRYSTALLIZATION_OUTPUT_SUMMARY_FIELDS}
  query GetKnowledge($knowledgeId: String!) {
    knowledge(knowledgeId: $knowledgeId) {
      ...
      relationshipAssignment {
        ...KnowledgeRelationshipAssignmentFields
      }
      crystallizationOutput {
        ...CrystallizationOutputSummaryFields
      }
      publicationOrigin {
        schemaVersion
        kind
        ...
        committeeCircleId
        submittedByPubkey
        submitAuthoritySemantics
      }
      publicationOriginDigest
      publicationState
      publicationLicenseRef
      publicationLicenseVersion
      publicationLicenseDigest
      publicationSourceSnapshotDigest
      stablePublicPath
      publicReleaseAuthorizedAt
      publicationVersions(limit: 20) {
        version
        state
        targetCircleId
        stablePublicPath
      ...
      crystalReceipts(limit: 12) {
        ...
        ownerDisplaySource
        ownerCircleAlias
        ownerNeedsDisplayDisambiguation
        ownerUserId
        contributionRole
...
export const GET_KNOWLEDGE_RELATIONSHIP_LABELS = gql`
  ${KNOWLEDGE_RELATIONSHIP_LABEL_FIELDS}
  query GetKnowledgeRelationshipLabels($status: String = "active") {
    knowledgeRelationshipLabels(status: $status) {
      ...KnowledgeRelationshipLabelFields
    }
  }
`;

export const MARK_MESSAGE_USEFUL = gql`
  mutation MarkMessageUseful($circleId: Int!, $envelopeId: String!) {
    markMessageUseful(circleId: $circleId, envelopeId: $envelopeId) {
      ok
      usefulCount
      isFeatured
      viewerHasMarkedUseful
      changed
    }
  }
`;

export const UNMARK_MESSAGE_USEFUL = gql`
  mutation UnmarkMessageUseful($circleId: Int!, $envelopeId: String!) {
    unmarkMessageUseful(circleId: $circleId, envelopeId: $envelopeId) {
      ok
      usefulCount
      isFeatured
      viewerHasMarkedUseful
