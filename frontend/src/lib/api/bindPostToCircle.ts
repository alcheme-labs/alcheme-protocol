...
export async function bindPostToCircle(input: {
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
        const response = await authenticatedApiFetch(`${baseUrl}/api/v1/posts/${encodeURIComponent(input.contentId)}/circle`, {
            method: 'POST',
            headers: {
        ...
        if (response.ok) {
            ...
            if (
                ...
                throw new Error(ONCHAIN_WRITE_ERROR_CODES.bindCircleAuthorityMissing);
            }
        ...
        throw new Error(`${ONCHAIN_WRITE_ERROR_CODES.bindCircleFailed}: ${lastError}`);
    }

    throw new Error(`${ONCHAIN_WRITE_ERROR_CODES.bindCircleTimeout}: ${lastError || 'post not indexed yet'}`);
}
