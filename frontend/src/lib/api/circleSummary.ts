...
import {
    ...
    pickCircleSummaryTopologyPayload,
    pickCircleCognitiveMapAiView,
...
export interface CircleSummarySnapshotResult {
...
export async function fetchCircleSummarySnapshot(input: {
    circleId: number;
    regenerate?: boolean;
}): Promise<CircleSummarySnapshotResult> {
    const url = new URL(
    ...
    const response = await authenticatedApiFetch(url.toString(), {
        method: 'GET',
        cache: 'no-store',
    } as any);
    const payload = await response.json().catch(() => null) as JsonPayload;
    ...
    return {
        snapshot: snapshotPayload ? pickCircleSummarySnapshot(snapshotPayload) : null,
        topology: pickCircleSummaryTopologyPayload(payload?.topology),
        cognitiveMapAi: pickCircleCognitiveMapAiView(payload?.cognitiveMapAi),
    };
}
...
export async function fetchCircleSummaryKnowledgeOutputs(input: {
    ...
    const response = await authenticatedApiFetch(getGraphqlUrl(), {
        method: 'POST',
        headers: {
