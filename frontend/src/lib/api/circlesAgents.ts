...
export async function fetchCircleAgents(circleId: number, signal?: AbortSignal): Promise<CircleAgentRecord[]> {
...
export async function fetchCircleAgentPolicy(circleId: number, signal?: AbortSignal): Promise<CircleAgentPolicy> {
    const route = await resolveNodeRoute('circle_agents');
    const baseUrl = route.urlBase;
