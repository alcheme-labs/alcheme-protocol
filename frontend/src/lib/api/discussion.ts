...
export async function sendDiscussionMessage(input: {
    ...
    const response = await apiFetch(`${baseUrl}/api/v1/discussion/circles/${input.circleId}/messages`, {
        method: 'POST',
        headers,
        body: JSON.stringify({
            senderPubkey: input.senderPubkey,
...
export async function createDiscussionSession(input: {
    ...
    const response = await apiFetch(`${baseUrl}/api/v1/discussion/sessions`, {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
        },
