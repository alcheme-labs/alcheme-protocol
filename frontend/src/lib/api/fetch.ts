...
function normalizeApiFetchOptions(options: ApiFetchInputOptions): ApiFetchOptions {
  return 'init' in options
...
export async function apiFetch(
  ...
  const normalizedOptions = normalizeApiFetchOptions(options);
...
export async function authenticatedApiFetchJson<T = any>(
  input: RequestInfo | URL,
  options: ApiFetchInputOptions = {},
): Promise<T> {
  const response = await authenticatedApiFetch(input, options);
  if (!response.ok) {
    const body = await response.text();
    throw new Error(`${response.status} ${body}`);
  }
