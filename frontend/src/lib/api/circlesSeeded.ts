...
export async function importSeededSources(
    ...
    const response = await authenticatedApiFetch(`${route.urlBase}/api/v1/circles/${circleId}/seeded/import`, {
        method: 'POST',
        headers: {
...
export async function fetchSeededFileTree(circleId: number): Promise<SeededFileTreeNode[]> {
    const route = await resolveNodeRoute('seeded');
    const response = await authenticatedApiFetch(`${route.urlBase}/api/v1/circles/${circleId}/seeded/tree`, {
        method: 'GET',
        cache: 'no-store',
    });
