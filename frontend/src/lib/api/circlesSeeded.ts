...
export async function importSeededSources(
    ...
    const response = await apiFetch(`${route.urlBase}/api/v1/circles/${circleId}/seeded/import`, {
        method: 'POST',
        credentials: 'include',
        headers: {
...
export async function fetchSeededFileTree(circleId: number): Promise<SeededFileTreeNode[]> {
    const route = await resolveNodeRoute('seeded');
    const response = await apiFetch(`${route.urlBase}/api/v1/circles/${circleId}/seeded/tree`, {
        method: 'GET',
        credentials: 'include',
        cache: 'no-store',
    });
