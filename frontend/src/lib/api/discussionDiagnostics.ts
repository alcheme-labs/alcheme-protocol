...
async function resolveDiscussionDiagnosticsBaseUrl(): Promise<string> {
    return (await resolveNodeRoute('discussion_runtime')).urlBase;
}
...
export async function fetchDiscussionAnalysisDiagnostics(
    ...
    const baseUrl = await resolveDiscussionDiagnosticsBaseUrl();
    const response = await authenticatedApiFetch(
        `${baseUrl}/api/v1/discussion/admin/messages/${encodeURIComponent(normalizedEnvelopeId)}/analysis`,
        {
            method: 'GET',
            headers: {
...
export async function reanalyzeDiscussionMessage(envelopeId: string): Promise<{ jobId: number; status: string }> {
    ...
    const baseUrl = await resolveDiscussionDiagnosticsBaseUrl();
    const response = await authenticatedApiFetch(
        `${baseUrl}/api/v1/discussion/admin/messages/${encodeURIComponent(normalizedEnvelopeId)}/reanalyze`,
        {
            method: 'POST',
            headers: {
                Accept: 'application/json',
...
export async function fetchDiscussionSummaryDiagnostics(
    ...
    const baseUrl = await resolveDiscussionDiagnosticsBaseUrl();
    const response = await authenticatedApiFetch(
        `${baseUrl}/api/v1/discussion/admin/circles/${encodeURIComponent(normalizedCircleId)}/summary`,
        {
            method: 'GET',
            headers: {
                Accept: 'application/json',
...
export async function fetchDiscussionTriggerDiagnostics(
    ...
    const baseUrl = await resolveDiscussionDiagnosticsBaseUrl();
    const response = await authenticatedApiFetch(
        `${baseUrl}/api/v1/discussion/admin/circles/${encodeURIComponent(normalizedCircleId)}/trigger`,
        {
            method: 'GET',
            headers: {
                Accept: 'application/json',
