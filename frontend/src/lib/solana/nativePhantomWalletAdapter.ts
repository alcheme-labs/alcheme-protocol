...
import {
    ...
    buildNativePhantomSignMessageRequestId,
...
export class NativePhantomWalletAdapter extends BaseMessageSignerWalletAdapter<typeof NativePhantomWalletName> {
    ...
    private async requestProviderPayload(
        ...
        const requestId = method === 'signMessage' && typeof payload.message === 'string'
            ? await buildNativePhantomSignMessageRequestId(bs58.decode(payload.message))
            : createNativePhantomRequestId(method);
    ...
    private readSessionRecord(): NativePhantomSessionRecord | null {
        ...
        return readNativePhantomSessionRecord();
    }
...
export async function resumeNativePhantomSignMessageCallback(input: {
    callbackUrl: string;
    signedMessage: string;
    expectedWalletPublicKey: string;
}): Promise<Uint8Array | null> {
    const expectedRequestId = await buildNativePhantomSignMessageRequestId(input.signedMessage);
    const callbackUrl = new URL(input.callbackUrl);
    if (callbackUrl.searchParams.get('request_id') !== expectedRequestId) {
        return null;
    }
    const sessionRecord = readNativePhantomSessionRecord();
    if (!sessionRecord) {
        throw new Error('Native Phantom session is unavailable for callback recovery.');
    }
    if (sessionRecord.walletPublicKey !== input.expectedWalletPublicKey) {
        throw new Error('Native Phantom session wallet changed during callback recovery.');
    }
    const result = decryptPhantomProviderCallback({
    ...
    if (typeof signature !== 'string') {
        throw new Error('Recovered Phantom callback has no message signature.');
    }
    return bs58.decode(signature);
}

function readNativePhantomSessionRecord(): NativePhantomSessionRecord | null {
