...
import {
    readCircleShortcuts,
    resolveCircleShortcutEntryHref,
...
export default function BottomNav() {
    ...
    const handleCircleTabClick = useCallback((event: MouseEvent<HTMLAnchorElement>) => {
        if (event.defaultPrevented || event.metaKey || event.ctrlKey || event.shiftKey || event.altKey) {
            return;
        }

        const nextHref = resolveCircleShortcutEntryHref({
            state: readCircleShortcuts(),
            fallbackHref: '/circles',
            hasExplicitIntent: false,
        });
        if (nextHref === '/circles') return;
                ...
                {NAV_ITEMS.map(({ href, key, icon: Icon, ...rest }) => {
                    ...
                    return (
                        <Link
                            ...
                            onClick={key === 'circles' ? handleCircleTabClick : undefined}
                        >
