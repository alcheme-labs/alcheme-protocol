...
import type { WalletImpactSummary } from '@/lib/circles/createCircleWalletImpact';
...
export default function RegisterIdentitySheet({
    ...
    const identityHandleValidationCopy: IdentityHandleValidationCopy = useMemo(() => ({
    ...
    const handleSubmit = async () => {
        ...
        if (validationError) {
            setLocalError(formatIdentityHandleValidationError(validationError, identityHandleValidationCopy));
            return;
        }
        setLocalError(null);
                ...
                >
                    ...
                    >
                        ...
                        <div className={styles.body}>
                            ...
                            {walletImpact && walletImpact.totalPromptCount > 0 && (
                                <div className={styles.walletImpact}>
                                    <div className={styles.walletImpactHeader}>
                                        <span>{t('walletImpact.title')}</span>
                                        <strong>{t('walletImpact.total', { count: walletImpact.totalPromptCount })}</strong>
                                    </div>
                                    <div className={styles.walletImpactList}>
                                        {walletImpact.items.filter((item) => item.required).map((item) => (
                                            <span key={item.id} className={styles.walletImpactItem}>
