...
export default function KnowledgeDiscussionPanel({
    ...
    const { sessionUser } = useIdentityOnboarding();
    ...
    useEffect(() => {
        ...
        try {
            ...
            if (
                ...
                || parsed.senderPubkey !== viewerSessionPubkey
    ...
    }, [circleId, discussionSessionStorageKey, viewerSessionPubkey]);
    ...
    const ensureDiscussionSessionToken = useCallback(async (): Promise<string | null> => {
        ...
        if (!viewerSessionPubkey) throw new Error(t('errors.connectWalletFirst'));
        ...
        sessionBootstrapRef.current = (async () => {
            ...
            const created = await createDiscussionSession({
                senderHandle: t('message.unknownAuthor'),
    ...
    }, [circleId, discussionSession, persistDiscussionSession, t, useSessionTokenAuth, viewerSessionPubkey]);
    ...
    const handleSubmit = useCallback(async () => {
        ...
        if (!viewerSessionPubkey) {
            requestWalletConnection({ source: 'knowledge_discussion' });
            return;
        }
                ...
                <div className={styles.composer}>
                    ...
                    <div className={styles.composerFooter}>
                        <span className={styles.composerStatus}>
                            {!viewerSessionPubkey ? t('composer.walletRequired') : t('composer.boundHint')}
                        </span>
                        <div className={styles.composerActions}>
                            {!viewerSessionPubkey && (
                                <button
                                    ...
                                    onClick={() => requestWalletConnection({ source: 'knowledge_discussion' })}
                                >
