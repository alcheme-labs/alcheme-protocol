...
import { formatInternalRecordVersionLabel } from '@/lib/knowledge/relationshipLabels';
...
export default function KnowledgeVersionDiffPanel({
    ...
    const versionOptions = useMemo(
        () => availableVersions.map((version) => ({
            value: version,
            label: formatInternalRecordVersionLabel(version, locale) ?? String(version),
        })),
        [availableVersions, locale],
    );
                            ...
                            <div className={styles.grid}>
                                <div className={styles.snapshot}>
                                    <h3 className={styles.snapshotTitle}>
                                        {formatInternalRecordVersionLabel(diff.fromSnapshot.version, locale) ?? String(diff.fromSnapshot.version)}
                                    </h3>
                                    <div className={styles.snapshotMeta}>
                                        <span>{t('snapshot.eventType', {value: diff.fromSnapshot.eventType})}</span>
                                        <span>{t('snapshot.actor', {value: diff.fromSnapshot.actorHandle || t('snapshot.system')})}</span>
                                ...
                                <div className={styles.snapshot}>
                                    <h3 className={styles.snapshotTitle}>
                                        {formatInternalRecordVersionLabel(diff.toSnapshot.version, locale) ?? String(diff.toSnapshot.version)}
                                    </h3>
                                    <div className={styles.snapshotMeta}>
                                        <span>{t('snapshot.eventType', {value: diff.toSnapshot.eventType})}</span>
                                        <span>{t('snapshot.actor', {value: diff.toSnapshot.actorHandle || t('snapshot.system')})}</span>
