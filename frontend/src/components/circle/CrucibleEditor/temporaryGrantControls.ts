...
export interface TemporaryGrantControlsState {
    ...
    viewerExpiredGrant: TemporaryEditGrantControlView | null;
...
export function resolveTemporaryGrantControls(
    ...
    const viewerExpiredGrant = blockGrants.find((grant) =>
        grant.granteeUserId === input.viewerUserId && grant.status === 'expired',
    ) || null;
