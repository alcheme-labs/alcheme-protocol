...
function readEditorMarkdown(editor: Editor, singleParagraph: boolean): string {
    ...
    if (
        ...
        // TipTap serializes this structural empty tail as `&nbsp;`; preserve all real NBSP text.
...
export default function CollaborativeEditor({
    ...
    const handleUpdate = useCallback(({ editor }: { editor: Editor }) => {
        const markdown = readEditorMarkdown(editor, singleParagraph);
        if (!singleParagraph || markdown.trim()) {
            onUpdate?.(markdown);
        }
        syncActiveReferenceQuery(editor);
    }, [onUpdate, singleParagraph, syncActiveReferenceQuery]);
    ...
    const editor = useEditor({
        extensions: [
            StarterKit.configure({
                undoRedo: false,
                codeBlock: singleParagraph ? false : {},
            }),
            Highlight.configure({
                multicolor: false,
            }),
        ...
        editorProps: {
            handleDOMEvents: {
                keydown: (_view, event) => (
                    singleParagraph && event.key === 'Enter' && event.isComposing
                ),
            },
            handleKeyDown: (_view, event) => {
                if (singleParagraph && event.key === 'Enter' && !event.isComposing) {
    ...
    useEffect(() => {
        ...
        const currentMarkdown = readEditorMarkdown(editor, singleParagraph);
        if (
            currentMarkdown === normalizedInitialContent
            || (editor.isFocused && (
                editor.isEmpty
                || currentMarkdown.trimEnd() === normalizedInitialContent
            ))
        ) return;
    ...
    }, [editor, emitSelectionParagraph, initialContent, singleParagraph]);
    ...
    return (
        <div className={`${styles.editorWrapper} ${!editable ? styles.readOnly : ''} ${compact ? styles.editorWrapperCompact : ''}`.trim()}>
            {editable && <EditorToolbar editor={editor} t={t} singleParagraph={singleParagraph} />}
