...
interface InviteMemberSheetProps {
    ...
    /** Locally sourced candidates; auxiliary circles use their parent-member pool. */
    users: InvitableUser[];
    /** Optional directory search for independently managed main circles. */
    onSearch?: (query: string) => Promise<InvitableUser[]>;
    onClose: () => void;
...
export default function InviteMemberSheet({
    ...
    const [remoteUsers, setRemoteUsers] = useState<InvitableUser[]>([]);
    ...
    const filtered = onSearch ? remoteUsers : locallyFiltered;
    const emptyState = onSearch && search.trim().length < 2
        ? t('states.searchPrompt')
        : searching
            ? t('states.searching')
            : searchError
                ? t('states.searchFailed')
                : t('states.empty');
