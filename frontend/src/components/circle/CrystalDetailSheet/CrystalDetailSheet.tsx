...
import { computeCrystalVisualParams, type CrystalDataInput, type FrozenCrystalParams } from '@/lib/crystal/visualParams';
import {
...
import type { GQLKnowledgeRelationshipAssignment, GQLKnowledgeRelationshipLabel } from '@/lib/apollo/types';
...
export interface CrystalDetail {
    ...
    relationshipLabelKey: string;
    relationshipAssignment?: GQLKnowledgeRelationshipAssignment | null;
    sourceDraftVersionLabel?: string | null;
    internalRecordVersion: number;
    version: string;
    citedBy: number;
    ageDays: number;
    ...
    crystalParams?: FrozenCrystalParams | null;
    /** Original knowledge creation timestamp for dynamic patina */
    createdAt?: string;
    /** Dynamic heat score [0-100] for warmth */
    heatScore?: number;
}
...
export default function CrystalDetailSheet({
    ...
    useEffect(() => {
        if (!open || !crystal?.knowledgeId) {
            ...
            setRelationshipLabelSheetOpen(false);
            return;
        }
    ...
    const outputView = useMemo(() => {
        ...
        return buildCrystalOutputViewModelFromRecord({
            knowledge: {
                ...
                title: crystal.title,
                version: crystal.internalRecordVersion || parseInt(crystal.version.replace('v', ''), 10) || 1,
                relationshipAssignment: crystal.relationshipAssignment,
    ...
    const hasHistoricalFallbackContributor = crystal.contributors.some((item) => item.assessmentKind === 'HISTORICAL_FALLBACK');
    const hasMockContributor = crystal.contributors.some((item) => item.assessmentKind === 'MOCK');
    const formatContributorWeight = (contributor: CrystalContributor): string => {
        if (contributor.assessmentKind === 'HISTORICAL_FALLBACK') return t('ledger.historicalFallbackWeight');
        if (contributor.assessmentKind === 'MOCK') return t('ledger.mockWeight');
        if (!contributor.assessmentKind || contributor.assessmentKind === 'UNKNOWN') return t('ledger.unverifiedWeight');
        return `${Math.round(contributor.weight * 100)}%`;
    };
    const resolveContributorWeightBarWidth = (contributor: CrystalContributor): string => {
        if (
            !contributor.assessmentKind
            || contributor.assessmentKind === 'HISTORICAL_FALLBACK'
            || contributor.assessmentKind === 'MOCK'
            || contributor.assessmentKind === 'UNKNOWN'
        ) {
            return '0%';
        }
        return `${Math.round(contributor.weight * 100)}%`;
    ...
    return (
        <>
                    ...
                    >
                        ...
                        <div className={styles.scrollArea}>
                            ...
                            {crystal.knowledgeId && (() => {
                                ...
                                const params = computeCrystalVisualParams(input, crystal.crystalParams ?? null);
                                return (
                                        ...
                                        <div className={styles.lineageHead}>
                                            ...
                                            <span className={styles.lineageValue}>
                                                {hasHistoricalFallbackContributor
                                                    ? t('lineage.historicalFallbackSnapshot')
                                                    : hasMockContributor
                                                        ? t('lineage.mockSnapshot')
                                                        : snapshotContributor
                                            ...
                                            <div key={role} className={styles.ledgerGroup}>
                                                <div className={styles.ledgerRole}>
                                                    ...
                                                    <span className={styles.ledgerRoleWeight}>
                                                        {members.some((member) => member.assessmentKind === 'HISTORICAL_FALLBACK')
                                                            ? t('ledger.historicalFallbackWeight')
                                                            : members.some((member) => member.assessmentKind === 'MOCK')
                                                                ? t('ledger.mockWeight')
                                                                : members.some((member) => !member.assessmentKind || member.assessmentKind === 'UNKNOWN')
                                                                    ? t('ledger.unverifiedWeight')
                                                        ...
                                                        <div key={i} className={styles.ledgerMember}>
                                                            ...
                                                            <div className={styles.ledgerWeightBar}>
                                                                <div
                                                                    ...
                                                                    style={{ width: resolveContributorWeightBarWidth(c) }}
                                                                />
                                                            </div>
            ...
            <KnowledgeRelationshipLabelSheet
                open={open && relationshipLabelSheetOpen}
                labels={relationshipLabels}
                loading={relationshipLabelsLoading}
                failed={Boolean(relationshipLabelsError)}
                activeLabelKey={crystal.relationshipLabelKey}
