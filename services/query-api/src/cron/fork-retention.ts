...
export async function runForkRetentionCronPass(prisma: PrismaClient): Promise<void> {
...
export function startForkRetentionCron(
    prisma: PrismaClient,
    ...
    const run = async () => {
        try {
            await runSingletonTask(prisma, {
                taskKey: 'fork_retention',
                ownerId,
                leaseMs: 30 * 60 * 1000,
                minIntervalMs: 5 * 60 * 60 * 1000,
            }, () => runForkRetentionCronPass(prisma));
        } catch (error) {
