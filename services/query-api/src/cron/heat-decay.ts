...
import { runSingletonTaskTransaction } from '../services/runtime/queryRuntimeTaskState';
...
export async function runHeatDecayPass(prisma: PrismaClient | Prisma.TransactionClient): Promise<void> {
...
export function startHeatDecayCron(
    prisma: PrismaClient,
    ...
    const decay = async () => {
        try {
            await runSingletonTaskTransaction(prisma, {
                taskKey: 'heat_decay',
                ownerId,
                leaseMs: 10 * 60 * 1000,
                minIntervalMs: 55 * 60 * 1000,
            }, (tx) => runHeatDecayPass(tx));
        } catch (error) {
