...
export const VOICE_SESSION_MODERATION_BOUNDARY = Object.freeze({
...
export function voiceRouter(
  ...
  router.get("/sessions/:sessionId/participants", async (req, res, next) => {
    try {
      ...
      const participantDtos = await mapVoiceParticipants(prisma, voiceSession, participants);
      res.json({
        ok: true,
        sessionId: voiceSession.id,
        ...
        moderationBoundary: VOICE_SESSION_MODERATION_BOUNDARY,
      });
    } catch (error) {
      next(error);
    }
...
async function mapVoiceParticipants(
  prisma: VoicePrisma,
  voiceSession: Pick<VoiceSessionRow, "roomKey">,
  participants: VoiceParticipantRow[],
) {
  let queuedPosition = 0;
  const circleId = resolveVoiceDisplayCircleId(voiceSession);
  const displays = await resolveCircleActorDisplays({
    prisma,
    actors: participants.map((participant) => ({
      displayKey: `voice:${participant.walletPubkey}`,
      pubkey: participant.walletPubkey,
      circleId,
    })),
    mode: "current",
    locale: "en",
  ...
  return participants.map((participant) => {
    ...
    const display = displays.get(`voice:${participant.walletPubkey}`) ?? null;
    return {
      walletPubkey: participant.walletPubkey,
      displayName: display?.globalDisplayName ?? null,
      circleAlias: display?.circleAlias ?? null,
      effectiveDisplayName: display?.effectiveName ?? "A member",
      displaySource: display?.displaySource ?? "generic_member",
      displayCircleId: display?.displayCircleId ?? circleId,
...
function parseCircleIdFromRoomKey(roomKey: string): number | null {
...
export function resolveVoiceDisplayCircleId(
  voiceSession: Pick<VoiceSessionRow, "roomKey" | "room">,
): number | null {
  const directCircleId = parseCircleIdFromRoomKey(voiceSession.roomKey);
  if (directCircleId) return directCircleId;

  if (!voiceSession.roomKey.startsWith("external:")) return null;
  const parentCircleId = Number(voiceSession.room?.parentCircleId ?? 0);
  return Number.isSafeInteger(parentCircleId) && parentCircleId > 0
    ? parentCircleId
    : null;
}
