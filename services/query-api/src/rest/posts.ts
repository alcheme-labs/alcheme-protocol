...
import {
    readActivePlatformSafetyLegalStatuses,
    readPlatformSafetyPublicLegalStatus,
...
function attachPlatformSafetyLegalStatus(posts: any[], legalStatuses: any[]): any[] {
    const byContentId = new Map(legalStatuses.map((status) => [status.contentId, status]));
    return posts
        .filter((post) => byContentId.get(post.contentId)?.legalSafeProjection?.listRead !== 'excluded')
        .map((post) => {
...
export function postRouter(prisma: PrismaClient, redis: Redis): Router {
    ...
    router.get('/feed', async (req, res, next) => {
        try {
            ...
            res.json(toJsonPayload(attachPlatformSafetyLegalStatus(posts, legalStatuses)));
        } catch (error) {
            next(error);
        }
    });
    ...
    router.get('/:contentId', async (req, res, next) => {
        try {
            ...
            if (resolvedPost.safetyQuarantined === true) {
                return res.status(404).json({ error: 'Post not found' });
            }

            const legalStatus = await readPlatformSafetyPublicLegalStatus(
                prisma as any,
                resolvedPost.contentId,
            );
            if (legalStatus?.legalSafeProjection?.ordinaryRead === 'legal_tombstone') {
                return res.status(451).json({
                    error: 'Post unavailable',
                    reason: 'platform_safety_legal_tombstone',
                    platformSafetyLegalStatus: legalStatus,
                });
            }

            const responsePayload = toJsonPayload(legalStatus
                ? { ...resolvedPost, platformSafetyLegalStatus: legalStatus }
                : resolvedPost);

            // 帖子不可变,永久缓存
            await redis.setex(cacheKey, 3600, stringifyCachePayload(responsePayload));
