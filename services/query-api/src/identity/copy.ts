...
export function buildMemberElderEligibilityReason(
    ...
    if (reputationPercentile === null || reputationPercentile === undefined) {
        return '继续保持贡献与信誉，有机会晋升为资深贡献者。';
    }
    if (reputationPercentile <= elderPercentile) {
        return `当前信誉位于前 ${reputationPercentile}%（阈值前 ${elderPercentile}%）可晋升为资深贡献者。`;
    }
    return `当前信誉位于前 ${reputationPercentile}%（需进入前 ${elderPercentile}%）方可晋升为资深贡献者。`;
}

export function buildMemberElderPromotionReason(reputationPercentile: number, elderPercentile: number): string {
    return `当前信誉位于前 ${reputationPercentile}%（阈值前 ${elderPercentile}%），已晋升为资深贡献者。`;
}
...
export function buildIdentityHint(input: {
    ...
    return localizeIdentityReason('已处于资深贡献者层级，保持活跃可维持当前身份。', locale);
}

export function buildVisitorDustHint(locale: AppLocale): string {
    if (locale === 'en') {
        return 'Observers can send dust messages, but they do not enter the formal settlement flow.';
    }
    if (locale === 'es') {
        return 'Los observadores pueden enviar mensajes efímeros, pero no entran en el flujo formal de asentamiento.';
    }
    if (locale === 'fr') {
        return 'Les observateurs peuvent envoyer des messages éphémères, mais ils n’entrent pas dans le flux formel de consolidation.';
    }
    return '观察者可发烟尘消息，但不进入正式沉淀链路。';
}
...
function buildIdentityReasonMetadata(reason: string | undefined): IdentityReasonMetadata | null {
    ...
    const messagePromotion = reason.match(/^已发送 (\d+) 条消息，达到 (\d+) 条门槛，已晋升为(?:入局者|参与者)。$/);
    if (messagePromotion) {
        return {
    ...
    const reputationPromotion = reason.match(/^当前信誉位于前 ([\d.]+)%（阈值前 ([\d.]+)%），已晋升为(?:长老|资深贡献者)。$/);
