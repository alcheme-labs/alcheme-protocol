...
export function evaluateIdentity(
    ...
    if (currentLevel === IdentityLevel.Elder) {
        ...
        if (circleReputationPercentile !== undefined && circleReputationPercentile > thresholds.elderPercentile) {
            ...
            reason = `当前信誉已降至前 ${circleReputationPercentile}% 之外（阈值前 ${thresholds.elderPercentile}%），身份调整为贡献者。`;
        }
    }
    ...
    if (currentLevel === IdentityLevel.Member || currentLevel === IdentityLevel.Elder) {
        ...
        if (stats.lastActiveAt) {
            ...
            if (daysSinceActive > thresholds.inactivityDays) {
                ...
                reason = `已 ${daysSinceActive} 天未活跃（阈值 ${thresholds.inactivityDays} 天），身份调整为参与者。`;
            }
        }
    }
...
export async function evaluateAndUpdate(
    ...
    if (result.changed) {
        ...
        await prisma.$transaction(async (tx) => {
            if (typeof (tx.circleMember as any).updateMany === 'function') {
                const updateResult = await tx.circleMember.updateMany({
                    where: {
                        circleId,
                        userId,
                        identityLevel: currentLevel,
                    },
                    data: { identityLevel: result.newLevel },
                });
                if (Number(updateResult.count ?? 0) !== 1) return;
            } else {
        ...
        if (!appliedTransition) {
            return {
                ...result,
                changed: false,
                reason: 'identity_level_already_changed',
            };
        }
    }
