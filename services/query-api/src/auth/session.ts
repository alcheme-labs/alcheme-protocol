...
export const CURRENT_PUBLIC_DEMO_ADMISSION_POLICY = Object.freeze({
    version: 'us-public-adult-demo-2026-07-22',
    jurisdiction: 'US' as const,
    minimumAge: 18,
    supportedRegions: ['US', 'OTHER'] as const,
    chinaAvailability: 'unavailable_pending_separate_compliance' as const,
});

export interface AuthSessionAdmissionBinding {
...
export interface AuthSessionRecord {
    ...
    issuedAt: string;
    expiresAt: string;
    publicDemoAdmission?: AuthSessionAdmissionBinding;
}

export interface PublicDemoAdmissionRequestPayload {
    policyVersion: string;
    jurisdiction: string;
    region: string;
...
export type PublicDemoAdmissionSignaturePayload = PublicDemoAdmissionRequestPayload & {
    jurisdiction: 'US';
    region: 'US' | 'OTHER' | 'CN';
    adultAttested: true;
    termsAccepted: true;
...
interface SessionLoginMessagePayload {
    ...
    domain?: string;
    publicDemoAdmission?: PublicDemoAdmissionSignaturePayload;
}
...
export function isPublicDemoAdmissionRequired(): boolean {
    const explicit = parseOptionalBoolean(process.env.PUBLIC_DEMO_ADMISSION_REQUIRED);
    return explicit ?? isPublicHardeningRequired(process.env);
}

function normalizeAdmissionBinding(input: unknown): AuthSessionAdmissionBinding | null {
    if (!input || typeof input !== 'object') return null;
    const policyVersion = String((input as { policyVersion?: unknown }).policyVersion || '').trim();
    const acceptanceDigest = String((input as { acceptanceDigest?: unknown }).acceptanceDigest || '').trim();
    if (
        policyVersion !== CURRENT_PUBLIC_DEMO_ADMISSION_POLICY.version
        || !/^[a-f0-9]{64}$/.test(acceptanceDigest)
    ) {
        return null;
    }
    return { policyVersion, acceptanceDigest };
}
...
export async function createAuthSession(
    ...
    const publicDemoAdmission = normalizeAdmissionBinding(input.publicDemoAdmission);
    if (isPublicDemoAdmissionRequired() && !publicDemoAdmission) {
...
export async function getAuthSession(redis: Redis, sessionId: string): Promise<AuthSessionRecord | null> {
    ...
    try {
        ...
        const publicDemoAdmission = normalizeAdmissionBinding(parsed.publicDemoAdmission);
        if (!Number.isFinite(userId) || userId <= 0) return null;
        if (!pubkey || !issuedAt || !expiresAt) return null;
        if (isPublicDemoAdmissionRequired() && !publicDemoAdmission) return null;
...
export async function createSessionLoginNonce(
    ...
    input: {
        publicKey: string;
        domain?: string;
        publicDemoAdmission?: PublicDemoAdmissionRequestPayload;
    },
    ...
    const payload: SessionLoginMessagePayload = {
        ...
        publicDemoAdmission: input.publicDemoAdmission
            ? normalizePublicDemoAdmission(input.publicDemoAdmission)
            : undefined,
    };
...
export function parseSessionLoginMessage(message: string): SessionLoginMessagePayload | null {
    ...
    try {
        ...
        const publicDemoAdmission = parsed.publicDemoAdmission
            ? normalizePublicDemoAdmission(parsed.publicDemoAdmission)
            : undefined;
...
function normalizePublicDemoAdmission(input: unknown): PublicDemoAdmissionSignaturePayload {
    if (!input || typeof input !== 'object' || Array.isArray(input)) {
        throw new Error('public_demo_admission_incomplete');
    }
    ...
    if (
        !policyVersion
        || jurisdiction !== 'US'
        || !['US', 'OTHER', 'CN'].includes(region)
        || record.adultAttested !== true
        || record.termsAccepted !== true
        || record.privacyAccepted !== true
        || record.safetyPolicyAccepted !== true
    ) {
        throw new Error('public_demo_admission_incomplete');
    }
    return {
        policyVersion,
        jurisdiction: 'US',
        region: region as PublicDemoAdmissionSignaturePayload['region'],
        adultAttested: true,
        termsAccepted: true,
        privacyAccepted: true,
        safetyPolicyAccepted: true,
