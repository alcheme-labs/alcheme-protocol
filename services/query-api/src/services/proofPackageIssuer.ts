...
export interface IssueDetachedDigestSignatureInput {
    digestHex: string;
    issuerKeyId?: string;
    issuerSecret?: string;
    issuedAt?: Date | string;
...
export interface IssueDetachedDigestSignatureResult {
    issuer_key_id: string;
    issued_signature: string;
    issued_at: string;
}
...
export function normalizeIssuerKeyId(value: string): string {
...
export function normalizeIsoDate(value: Date | string | undefined): string {
    if (!value) {
...
export function issueDetachedDigestSignature(
    input: IssueDetachedDigestSignatureInput,
): IssueDetachedDigestSignatureResult {
...
export function issueProofPackageSignature(
    ...
    const issued = issueDetachedDigestSignature({
        digestHex: buildProofBindingCanonicalDigest(canonicalMessage),
...
async function loadExistingProofPackageIssuance(
    prisma: PrismaClient,
    proofPackageId: bigint,
    issuerKeyId: string,
    issuedSignature: string,
): Promise<DraftProofPackageIssuanceRow | null> {
    const rows = await prisma.$queryRaw<DraftProofPackageIssuanceRow[]>(Prisma.sql`
        SELECT
            issuer_key_id AS "issuerKeyId",
            issued_signature AS "issuedSignature",
            issued_at AS "issuedAt"
        FROM draft_proof_package_issuances
        WHERE proof_package_id = ${proofPackageId}
          AND issuer_key_id = ${issuerKeyId}
          AND issued_signature = ${issuedSignature}
        ORDER BY id DESC
        LIMIT 1
    `);
    return rows[0] || null;
...
export async function persistProofPackageIssuance(
    ...
    const insertedIssuance = await prisma.$queryRaw<DraftProofPackageIssuanceRow[]>(Prisma.sql`
        ...
        ON CONFLICT (proof_package_id, issuer_key_id, issued_signature) DO NOTHING
        RETURNING
            issuer_key_id AS "issuerKeyId",
            issued_signature AS "issuedSignature",
            issued_at AS "issuedAt"
    `);
    const issuance = insertedIssuance[0]
        || await loadExistingProofPackageIssuance(
            prisma,
            proofPackage.id,
            issuerKeyId,
            input.issuedSignature,
