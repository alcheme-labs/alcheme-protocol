...
import {
    ...
    validateDraftContributorProofRecord,
    type DraftContributorProofRecord,
} from './contributorProof';
import { markSourceMaterialsCrystallized } from './sourceMaterials/lifecycleHooks';
...
export async function syncKnowledgeContributionsFromDraftProof(
    ...
    const proof = options?.contributorProof
        ? validateDraftContributorProofRecord(options.contributorProof, {
            draftPostId: input.draftPostId,
            circleId: knowledge.circleId,
            anchorId: options?.proofAnchorId,
        })
        : await getDraftContributorProof(
            db as PrismaClient,
            input.draftPostId,
    ...
    if (options?.proofAnchorId) {
        const expectedAnchor = normalizeHex(options.proofAnchorId);
        const proofAnchor = normalizeHex(proof.anchorId);
        if (!expectedAnchor || !proofAnchor || expectedAnchor !== proofAnchor) {
            throw new KnowledgeContributionSyncError(
                'proof_binding_required',
                409,
                'request proof snapshot source anchor does not match prepared draft proof',
            );
        }
    }
    ...
    const writeSnapshot = async (writer: PrismaLike) => {
        ...
        await markSourceMaterialsCrystallized(writer, {
            sourceMaterialIds: [],
            draftPostId: input.draftPostId,
