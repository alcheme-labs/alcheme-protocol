...
export function toAiJobRecord(row: any): AiJobRecord {
    return {
        ...
        taskType: row.taskType ?? row.task_type ?? null,
        taskCatalogVersion: row.taskCatalogVersion ?? row.task_catalog_version ?? null,
        contextCapsuleId: row.contextCapsuleId ?? row.context_capsule_id ?? null,
        capabilityTrace: toRecord(row.capabilityTrace ?? row.capabilityTraceJson ?? row.capability_trace_json),
...
export function toAiJobView(job: AiJobRecord): AiJobView {
    const {
        ...
        contextCapsuleId: _contextCapsuleId,
        capabilityTrace: _capabilityTrace,
        ...view
    } = job;
    return view;
}
