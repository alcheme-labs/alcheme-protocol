...
export function createAiJobWorker(input: {
    ...
    const reportPollingFailure = (error: unknown) => {
        const message = error instanceof Error ? error.message : String(error);
        console.error(`ai_job_worker_poll_failed:${message}`);
    };
    ...
    const runOnce = async (): Promise<boolean> => {
        ...
        activeRun = (async () => {
            ...
            try {
                ...
                clearRenewHandle();
                const normalizedError = normalizeAiJobHandlerError(error);
                await failAiJob(input.prisma as any, {
                    jobId: job.id,
                    claimToken: job.claimToken || '',
    ...
    return {
        ...
        start() {
            ...
            intervalHandle = setInterval(() => {
                void runOnce().catch(reportPollingFailure);
            }, pollIntervalMs);
            void runOnce().catch(reportPollingFailure);
        },
        async stop() {
