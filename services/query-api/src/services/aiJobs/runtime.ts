...
import { classifyAiJobCompatibility } from "../aiOperatingLayer/jobCompatibility";
...
const AI_JOB_TYPES = [
  ...
  "guardian_finding_generate",
  "source_grounded_ask_generate",
  "neutral_evaluation_generate",
  "circle_growth_advisor_generate",
] as const;
...
export async function requeueStaleAiJobs(
  ...
  })).filter((row: any) => row.jobType !== "crystal_asset_issue");
...
export async function enqueueAiJob(
  ...
  const prismaAny = prisma as any;
  if (input.jobType === "crystal_asset_issue") {
    throw new Error("crystal_asset_issuance_fenced");
  }
  const compatibility = classifyAiJobCompatibility(input.jobType);
  const taskType = input.taskType ?? compatibility.taskType;
  const taskCatalogVersion =
    input.taskCatalogVersion ?? compatibility.taskCatalogVersion;
  ...
  try {
    const created = await prismaAny.aiJob.create({
      data: {
        ...
        capabilityTraceJson: input.capabilityTrace || null,
