...
function compareExistingClaimOrder(left: { availableAt: Date; id: number }, right: { availableAt: Date; id: number }): number {
    const availableDiff = left.availableAt.getTime() - right.availableAt.getTime();
    if (availableDiff !== 0) return availableDiff;
    return left.id - right.id;
}

export function getAiJobClaimLeaseMs(): number {
    const parsed = Number(process.env.AI_JOB_CLAIM_LEASE_MS || DEFAULT_CLAIM_LEASE_MS);
...
export async function requeueStaleAiJobs(
    ...
    const terminalIds = (Array.isArray(staleJobs) ? staleJobs : [])
        .filter((row) => Number(row.attempts || 0) >= Number(row.maxAttempts || DEFAULT_MAX_ATTEMPTS))
    ...
    const terminal = terminalIds.length > 0
    ...
    const retryable = retryableIds.length > 0
        ? await prismaAny.aiJob.updateMany({
            where: {
                id: { in: retryableIds },
                status: 'running',
...
export async function enqueueAiJob(prisma: PrismaLike, input: EnqueueAiJobInput): Promise<AiJobRecord> {
    const prismaAny = prisma as any;
    ...
    try {
        const created = await prismaAny.aiJob.create({
            data: {
                ...
                status: 'queued',
                attempts: 0,
                maxAttempts: Math.max(1, Number(input.maxAttempts ?? DEFAULT_MAX_ATTEMPTS)),
...
export async function claimNextAiJob(prisma: PrismaLike, input: ClaimAiJobInput): Promise<AiJobRecord | null> {
    const prismaAny = prisma as any;
    const now = input.now ?? new Date();
    ...
    const nonDiscussionJobTypes = requestedJobTypes.filter(
        (jobType) => jobType !== DISCUSSION_TRIGGER_JOB_TYPE && jobType !== DISCUSSION_ANALYZE_JOB_TYPE,
    );
    ...
    const [nonDiscussionCandidate, triggerCandidate, analyzeCandidate] = await Promise.all([
        nonDiscussionJobTypes.length > 0
            ? prismaAny.aiJob.findMany({
                where: {
                    ...baseWhere,
    ...
    const candidates = [nonDiscussionCandidate, discussionCandidate].filter(Boolean).sort((left: any, right: any) => (
...
export async function completeAiJob(prisma: PrismaLike, input: CompleteAiJobInput): Promise<AiJobRecord | null> {
    const prismaAny = prisma as any;
    const now = input.now ?? new Date();
    const updated = await prismaAny.aiJob.updateMany({
        where: {
...
export async function failAiJob(prisma: PrismaLike, input: FailAiJobInput): Promise<AiJobRecord | null> {
    const prismaAny = prisma as any;
    const now = input.now ?? new Date();
