...
function buildBaseCrystallizationOutputQuery(
    ...
    options: { limitOne?: boolean } = { limitOne: true },
): Prisma.Sql {
    return Prisma.sql`
        SELECT
        ...
        ${whereClause}
        ${options.limitOne === false ? Prisma.empty : Prisma.sql`LIMIT 1`}
    `;
}
...
export interface CrystallizationOutputSummaryRecord {
    knowledgeId: string;
...
function buildCrystallizationOutputSummaryRecord(
    row: CrystallizationOutputRow | null | undefined,
): CrystallizationOutputSummaryRecord | null {
    if (!row?.knowledgeId) return null;
    return {
        knowledgeId: row.knowledgeId,
        sourceDraftPostId: row.sourceDraftPostId ?? null,
        sourceDraftVersion: row.sourceDraftVersion ?? null,
        sourceAnchorId: row.sourceAnchorId ?? null,
        sourceSummaryHash: row.sourceSummaryHash ?? null,
        sourceMessagesDigest: row.sourceMessagesDigest ?? null,
    };
}

export async function loadCrystallizationOutputRecordsByKnowledgeIds(
    prisma: PrismaClient,
    knowledgeIds: string[],
): Promise<Map<string, CrystallizationOutputSummaryRecord>> {
    const uniqueKnowledgeIds = Array.from(
        new Set(knowledgeIds.map((value) => value.trim()).filter(Boolean)),
    );
    if (uniqueKnowledgeIds.length === 0) {
        return new Map();
    }

    const rows = await prisma.$queryRaw<CrystallizationOutputRow[]>(
        buildBaseCrystallizationOutputQuery(
            Prisma.sql`WHERE k.knowledge_id IN (${Prisma.join(uniqueKnowledgeIds)})`,
            { limitOne: false },
        ),
    );

    const records = new Map<string, CrystallizationOutputSummaryRecord>();
    for (const row of rows) {
        const record = buildCrystallizationOutputSummaryRecord(row);
        if (record) {
