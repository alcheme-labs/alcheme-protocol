        ...
        async saveGrant(grant) {
            const rows = await prisma.$queryRaw<TemporaryEditGrantRow[]>(Prisma.sql`
                INSERT INTO temporary_edit_grants (
                    ...
                    ${grant.approvalMode},
                    ${grant.status},
                    ${grant.governanceRequestId ?? null},
...
export async function requestTemporaryEditGrant(
    ...
    if (approvalMode === 'governance_request' && !governanceRequestId) {
        throw new Error('temporary_edit_grant_governance_request_required');
    }
...
export async function reconcileTemporaryEditGrantGovernance(
    ...
    if (grant.approvalMode !== 'governance_request' || !grant.governanceRequestId || !input.governanceRequest) {
        return grant;
    }
    if (input.governanceRequest.id !== grant.governanceRequestId) {
        return grant;
    }
    ...
    if (governanceStatus === 'rejected' || governanceStatus === 'cancelled') {
        const now = new Date();
        return store.saveGrant({
            ...grant,
            status: 'rejected',
