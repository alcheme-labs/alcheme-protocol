...
import { splitDraftParagraphs } from './paragraphScope';
import { listDraftSourceParticipants } from './sourceParticipants';
...
function deriveViewerPermission(input: {
    ...
    sourceParticipantUserIds: number[];
    ...
    const sourceParticipantEditAllowed =
        activeMembership
        && input.documentStatus === 'drafting'
        && userId !== null
        && input.sourceParticipantUserIds.includes(userId);
    ...
    if (manager) {
        ...
        if (baseDraftEditAllowed && userId !== null && input.participantUserIds.includes(userId)) {
            permissionSources.push('block_discussion_participant');
        }
        if (sourceParticipantEditAllowed) {
            permissionSources.push('source_participant');
        }
        if (activeGrant) {
...
export async function resolveDraftBlockReadModel(
    ...
    const membership = input.viewerUserId && post.circleId !== null
        ...
        : null;
    const shouldProjectSourceParticipants =
        input.viewerUserId !== null
        && input.viewerUserId !== undefined
        && post.circleId !== null
        && lifecycle.documentStatus === 'drafting'
        && String(membership?.status || '') === 'Active';
    const sourceParticipantUserIds = shouldProjectSourceParticipants
        ? (await listDraftSourceParticipants(prisma, {
            draftPostId: input.draftPostId,
            circleId: post.circleId as number,
    ...
    const viewerPermissions = blocks.map((block) =>
        deriveViewerPermission({
            ...
            membership: membership
                ...
                : null,
            sourceParticipantUserIds,
            temporaryGrants,
            documentStatus: lifecycle.documentStatus,
            nowIso,
        }),
    );
