...
export interface CreateForkCircleInput {
    ...
    ensureContextCapsule?: (input: {
        declaration: ForkDeclarationRecord;
        lineage: CircleForkLineageRecord;
    }) => Promise<void>;
    createdAt?: Date;
}
    ...
    return {
        ...
        async saveDeclaration(declaration) {
            return prisma.$transaction(async (tx) => {
                const existing = await tx.forkDeclaration.findUnique({
                    where: { declarationId: declaration.declarationId },
                });
                if (existing && (
                    existing.sourceCircleId !== declaration.sourceCircleId
                    || existing.actorUserId !== declaration.actorUserId
                    || existing.declarationText !== declaration.declarationText
                )) {
                    throw new Error('fork_declaration_identity_mismatch');
                }
                if (declaration.status === 'attached') {
                    const competing = await tx.forkDeclaration.findFirst({
                        where: {
                            sourceCircleId: declaration.sourceCircleId,
                            declarationId: { not: declaration.declarationId },
                            status: { in: ['attached', 'reconciliation_pending'] },
                        },
                        select: { declarationId: true },
                    });
                    if (competing) throw new Error('fork_source_lifecycle_already_pending');
                }
                const saved = await tx.forkDeclaration.upsert({
                    where: { declarationId: declaration.declarationId },
                    create: {
                        ...
                        qualificationSnapshot: declaration.qualificationSnapshot as any,
                ...
                await projectForkSourceLifecycle(tx, declaration, existing?.status ?? null);
                return mapDeclarationRow(saved as ForkDeclarationRow);
            });
        },
...
async function projectForkSourceLifecycle(
    tx: Prisma.TransactionClient,
    declaration: ForkDeclarationRecord,
    previousStatus: string | null,
): Promise<void> {
    const circle = await tx.circle.findUnique({
        where: { id: declaration.sourceCircleId },
        select: { lifecycleStatus: true },
    });
    if (!circle) throw new Error('fork_source_circle_not_found');
    if (declaration.status === 'attached') {
        if (circle.lifecycleStatus === 'ForkPending') return;
        const pending = await tx.circle.updateMany({
            where: { id: declaration.sourceCircleId, lifecycleStatus: 'Active' },
            data: { lifecycleStatus: 'ForkPending' },
        });
        if (pending.count !== 1) throw new Error('fork_source_pending_projection_failed');
        return;
    }
    if (declaration.status === 'reconciliation_pending') {
        if (circle.lifecycleStatus !== 'ForkPending') {
            throw new Error('fork_source_reconciliation_state_mismatch');
        }
        return;
    }
    if (declaration.status !== 'completed') {
        throw new Error('fork_declaration_status_unsupported');
    }
    if (circle.lifecycleStatus === 'Active' && previousStatus === 'completed') return;
    const restored = await tx.circle.updateMany({
        where: { id: declaration.sourceCircleId, lifecycleStatus: 'ForkPending' },
        data: { lifecycleStatus: 'Active' },
    });
    if (restored.count !== 1) throw new Error('fork_source_active_projection_failed');
}
...
export async function createForkCircle(
    ...
    if (existingDeclaration && (existingLineage || existingDeclaration.status === 'completed')) {
        const completedDeclaration = existingDeclaration.status === 'completed'
            ? existingDeclaration
        ...
        if (existingLineage && input.ensureContextCapsule) {
            try {
                await input.ensureContextCapsule({
        ...
        return {
            ...
            contextPending: contextPending || undefined,
        };
    }
    ...
    let lineage: CircleForkLineageRecord;
    let completedDeclaration: ForkDeclarationRecord;
    try {
        lineage = await store.saveLineage({
        ...
        completedDeclaration = await store.saveDeclaration({
