...
import { lockDraftParagraphStructure } from './draftLifecycle/paragraphStructure';
...
export interface DraftDiscussionReviewContext {
...
export interface DraftDiscussionTimelineEventRecord {
    id: string;
    threadId: string;
    actorUserId: number;
    actorPubkey: string | null;
    action: 'create' | 'followup' | 'accept' | 'reject' | 'apply';
    targetVersion: number;
    createdAt: string;
}
...
interface DraftDiscussionTimelineEventRow {
    id: bigint;
    threadId: bigint;
    actorUserId: number;
    actorPubkey: string | null;
...
export async function createDraftDiscussionThread(
    ...
    return prisma.$transaction(async (tx) => {
        const locked = await lockDraftParagraphStructure(tx, input.draftPostId);
        if (
            !locked
            || locked.documentStatus !== 'review'
            || locked.currentSnapshotVersion !== targetVersion
        ) {
            throw new DraftDiscussionLifecycleError(
                'draft_discussion_snapshot_changed',
                409,
                'draft left the expected Review snapshot before the discussion thread was created',
            );
        }
...
export async function listDraftDiscussionTimelineEvents(
    prisma: PrismaClient,
    input: { draftPostId: number },
): Promise<DraftDiscussionTimelineEventRecord[]> {
    const rows = await prisma.$queryRaw<DraftDiscussionTimelineEventRow[]>(Prisma.sql`
        SELECT
            message.id,
            message.thread_id AS "threadId",
            message.author_id AS "actorUserId",
            actor.pubkey AS "actorPubkey",
            message.message_type AS action,
            thread.target_version AS "targetVersion",
            message.created_at AS "createdAt"
        FROM draft_discussion_messages message
        INNER JOIN draft_discussion_threads thread ON thread.id = message.thread_id
        LEFT JOIN users actor ON actor.id = message.author_id
        WHERE message.draft_post_id = ${input.draftPostId}
          AND thread.draft_post_id = ${input.draftPostId}
          AND message.message_type IN ('create', 'followup', 'accept', 'reject', 'apply')
    ...
    return rows.map((row) => ({
        ...
        actorPubkey: row.actorPubkey ? String(row.actorPubkey) : null,
        action: row.action as DraftDiscussionTimelineEventRecord['action'],
        targetVersion: Number(row.targetVersion),
        createdAt: row.createdAt.toISOString(),
    }));
}

export async function loadDraftDiscussionReviewContexts(
    prisma: PrismaClient,
    input: {
        draftPostId: number;
        threads: DraftDiscussionThreadRecord[];
    },
): Promise<Record<string, DraftDiscussionReviewContext>> {
    ...
    const currentVersion = Number(workflow?.currentSnapshotVersion);
    if (!Number.isSafeInteger(currentVersion) || currentVersion <= 0) {
        return unavailableReviewContexts(input.threads);
    }
    const versions = [...new Set([
        currentVersion,
        ...input.threads.map((thread) => thread.targetVersion),
    ])];
    const snapshots = await prisma.draftVersionSnapshot.findMany({
        where: {
            draftPostId: input.draftPostId,
            draftVersion: { in: versions },
        },
        select: {
            draftVersion: true,
    ...
    const contentByVersion = new Map(
        snapshots.map((snapshot) => [snapshot.draftVersion, snapshot.contentSnapshot]),
    );
    const currentContent = contentByVersion.get(currentVersion);
    return Object.fromEntries(input.threads.map((thread) => {
        const targetContent = contentByVersion.get(thread.targetVersion);
        if (targetContent === undefined || currentContent === undefined) {
            return [thread.id, unavailableReviewContext(currentVersion)];
        }
        const targetAtCreation = extractDiscussionTarget(targetContent, thread.targetType, thread.targetRef);
        const targetCurrent = extractDiscussionTarget(currentContent, thread.targetType, thread.targetRef);
        if (targetAtCreation === null || targetCurrent === null) {
            return [thread.id, unavailableReviewContext(currentVersion)];
        }
        return [thread.id, {
            state: targetAtCreation === targetCurrent ? 'unchanged' : 'changed',
            currentVersion,
            targetAtCreation,
            targetCurrent,
        } satisfies DraftDiscussionReviewContext];
    }));
}

function unavailableReviewContexts(
    threads: DraftDiscussionThreadRecord[],
): Record<string, DraftDiscussionReviewContext> {
    return Object.fromEntries(threads.map((thread) => [thread.id, unavailableReviewContext(null)]));
}

function unavailableReviewContext(currentVersion: number | null): DraftDiscussionReviewContext {
    return {
        state: 'unavailable',
        currentVersion,
        targetAtCreation: null,
        targetCurrent: null,
    };
}

function extractDiscussionTarget(
    content: string,
    targetType: DraftDiscussionTargetType,
    targetRef: string,
): string | null {
    if (targetType === 'document') return String(content || '').trim();
    const paragraphs = String(content || '')
        .split(/\n+/)
        .map((item) => item.replace(/\s+/g, ' ').trim())
        .filter(Boolean);
    const indices = targetRef.split(',').map((ref) => {
        const matched = ref.trim().match(/^paragraph:(\d+)$/i);
        return matched ? Number.parseInt(matched[1], 10) : Number.NaN;
    });
    if (
        indices.length === 0
        || indices.some((index) => !Number.isSafeInteger(index) || index < 0 || index >= paragraphs.length)
    ) {
        return null;
    }
    if (targetType === 'paragraph' && indices.length !== 1) return null;
    return indices.map((index) => paragraphs[index]).join('\n');
}
