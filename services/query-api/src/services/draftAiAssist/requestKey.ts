...
export function buildAcceptedIssueRevisionDedupeKey(input: {
    postId: number;
    requestedByUserId: number;
    acceptedThreadIds?: Array<number | string> | null;
    ...
    const fingerprint = stableStringify({
        ...
        acceptedThreadIds: normalizeThreadIds(input.acceptedThreadIds),
        targetParagraphIndex: Number.isFinite(Number(input.targetParagraphIndex))
            ? Number(input.targetParagraphIndex)
            : null,
        workingCopyHash: typeof input.workingCopyHash === 'string' && input.workingCopyHash.trim()
            ? input.workingCopyHash.trim()
            : null,
    ...
    return `accepted_issue_revision_generate:${crypto.createHash('sha256').update(fingerprint).digest('hex')}`;
}
