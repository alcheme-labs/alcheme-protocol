...
export function evaluateMembershipJoinDecision(input: {
    ...
    const requiresApproval =
        input.policy.joinRequirement === JoinRequirement.ApprovalRequired
        || input.policy.joinRequirement === JoinRequirement.TokenGated
        || input.policy.circleType === CircleType.Closed;
    // Legacy Crystal thresholds remain visible compatibility facts, never admission authority.
    const userCrystals = 0;
    const missingCrystals = 0;
