...
export async function loadKnowledgeVersionDiff(
    ...
    const actorDisplays = await resolveCircleActorDisplays({
        prisma: prisma as any,
        actors: [
            {
                displayKey: `knowledge-version:${input.fromVersion}:actor`,
                pubkey: fromEvent.actorPubkey,
                circleId: currentKnowledge.circleId,
            },
            {
                displayKey: `knowledge-version:${input.toVersion}:actor`,
                pubkey: toEvent.actorPubkey,
                circleId: currentKnowledge.circleId,
            },
        ],
        mode: 'current',
    ...
    const genericMember = localizeQueryApiCopy('identity.genericMember', locale);

    const fromSnapshot = buildSnapshot({
        knowledgeId,
        version: input.fromVersion,
        event: fromEvent,
        actorHandle: actorDisplays.get(`knowledge-version:${input.fromVersion}:actor`)?.effectiveName ?? null,
        currentKnowledge,
    });
    const toSnapshot = buildSnapshot({
        ...
        event: toEvent,
        actorHandle: actorDisplays.get(`knowledge-version:${input.toVersion}:actor`)?.effectiveName ?? null,
        currentKnowledge,
    });
    ...
    pushChange(fieldChanges, {
        ...
        fromValue: fromSnapshot.actorHandle ?? genericMember,
        toValue: toSnapshot.actorHandle ?? genericMember,
    });
    pushChange(fieldChanges, {
