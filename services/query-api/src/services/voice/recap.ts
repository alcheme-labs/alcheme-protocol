...
import { submitVoiceRecapAsSourceMaterial } from "../sourceMaterials/runtimeIntake";
import {
...
async function maybeCreateDraftSource(
  ...
  const material = await submitVoiceRecapAsSourceMaterial(prisma as any, {
    externalAppId: input.voiceSession.room?.externalAppId ?? null,
    ...
    evidencePrivacyClass: resolveVoiceRecapEvidencePrivacyClass(
      input.voiceSession.room?.metadata,
    ),
  });
...
function resolveVoiceRecapEvidencePrivacyClass(metadata: unknown): "circle_only" | "reviewer_only" | "sealed" {
  const record = plainObjectOrNull(metadata);
  const raw =
    record?.sourceMaterialPrivacyClass ??
    plainObjectOrNull(record?.voiceRecap)?.evidencePrivacyClass ??
    plainObjectOrNull(record?.voicePolicy)?.sourceMaterialPrivacyClass;
  const normalized = String(raw || "").trim().toLowerCase();
  if (normalized === "reviewer_only") return "reviewer_only";
