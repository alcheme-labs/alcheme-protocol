...
export interface DiscussionSigningPayload {
    ...
    subjectType?: 'knowledge' | 'discussion_message' | 'source_material' | 'announcement' | 'discussion_forward_bundle';
    subjectId?: string;
}
...
export function buildDiscussionRoomKey(circleId: number): string {
    // This key matches CommunicationRoom's circle key vocabulary, but Plaza
    // discussion still persists in circle_discussion_messages.
...
}): { subjectType?: 'knowledge' | 'discussion_message' | 'source_material' | 'announcement' | 'discussion_forward_bundle'; subjectId?: string } {
...
export async function ensureOffchainDiscussionSchema(prisma: PrismaClient): Promise<void> {
    const stmts = [
        ...
        CREATE TABLE IF NOT EXISTS offchain_peer_sync_state (
            ...
            peer_identity VARCHAR(256) NOT NULL DEFAULT 'legacy',
            last_remote_lamport BIGINT NOT NULL DEFAULT 0,
            last_success_at TIMESTAMP(3),
            last_error TEXT,
            updated_at TIMESTAMP(3) NOT NULL DEFAULT CURRENT_TIMESTAMP,
            PRIMARY KEY (peer_url, peer_identity)
        )
        `,
        `ALTER TABLE offchain_peer_sync_state ADD COLUMN IF NOT EXISTS peer_identity VARCHAR(256) NOT NULL DEFAULT 'legacy'`,
        `
        DO $$
        BEGIN
            IF EXISTS (
                SELECT 1
                FROM pg_constraint
                WHERE conrelid = 'offchain_peer_sync_state'::regclass
                  AND conname = 'offchain_peer_sync_state_pkey'
            ) AND NOT EXISTS (
                SELECT 1
                FROM pg_attribute a
                JOIN pg_index i
                ...
                WHERE i.indrelid = 'offchain_peer_sync_state'::regclass
                  AND i.indisprimary
                  AND a.attname = 'peer_identity'
            ) THEN
                ALTER TABLE offchain_peer_sync_state DROP CONSTRAINT offchain_peer_sync_state_pkey;
                ALTER TABLE offchain_peer_sync_state
                    ADD CONSTRAINT offchain_peer_sync_state_pkey PRIMARY KEY (peer_url, peer_identity);
            END IF;
            IF NOT EXISTS (
                SELECT 1
                FROM pg_constraint
