...
import {
    ...
    recordGovernedDraftCrystallizationAuthorization,
    getPersistedDraftWorkflowState,
    resolveDraftWorkflowState,
    findBriefLockingGovernanceCase,
    projectBriefLock,
    BRIEF_OPEN_PHASES,
    BRIEF_LOCK_PHASES,
...
import {
    ...
    recordGovernedCaseCrystallizationRoutingReceipt,
    type CrystallizationRoutingReceipt,
} from './versionSnapshots';
import { resolveRevisionDirectionCrystallizationRoute } from '../governance/draftGovernance';
import type { RevisionDirectionAcceptActionType } from '../governance/actionRegistry';
import { bindGovernanceCaseBrief } from '../governance/governanceCaseWorkflow';
...
export async function resolveDraftLifecycleReadModel(
    ...
    const stableSnapshot: DraftStableSnapshotView = currentSnapshotVersion === 1 && persistedStableSnapshot
        ? {
            ...
            crystallizationRoutingReceipt: persistedStableSnapshot.crystallizationRoutingReceipt,
    ...
    if (
        ['crystallization_active', 'crystallization_failed', 'crystallized'].includes(workflowState.documentStatus)
        && !stableSnapshot.crystallizationRoutingReceipt
    ) {
        warnings.push(
            'crystallization routing receipt is unavailable; publication path cannot be claimed',
        );
    }
    ...
    const briefLock = projectBriefLock(
        await findBriefLockingGovernanceCase(prisma, draftPostId, BRIEF_LOCK_PHASES),
    );
...
export async function enterDraftLifecycleCrystallization(
    ...
    const policyProfile = post.circleId
        ...
        : null;
    const lifecycleTemplate = policyProfile?.draftLifecycleTemplate || buildDefaultLifecycleTemplate();
    const currentPolicyProfileDigest = policyProfile
        ? computePolicyProfileDigest(buildPublicPolicyDigestSnapshot(policyProfile))
        : null;
    if (!currentPolicyProfileDigest || currentPolicyProfileDigest !== input.policyProfileDigest) {
        throw new DraftWorkflowStateError(
            'policy_profile_digest_stale',
            409,
            'the submitted policy profile digest is not the current Circle policy',
        );
    }
...
export async function routeDraftLifecycleToGovernanceCase(
    prisma: PrismaClient,
    input: {
        draftPostId: number;
        actorUserId: number;
    ...
    const lockingCase = await findBriefLockingGovernanceCase(prisma, draftPostId, BRIEF_OPEN_PHASES);
    if (lockingCase) {
        const workflow = typeof (prisma as any).$queryRaw === 'function'
            ? await getPersistedDraftWorkflowState(prisma, draftPostId)
            : null;
        const snapshot = workflow?.currentSnapshotVersion
            ? await loadDraftVersionSnapshot(prisma, {
                draftPostId,
            ...
            : null;
        if (
            workflow?.crystallizationAnchorSignature === input.anchorSignature
            && workflow.crystallizationPolicyProfileDigest === input.policyProfileDigest
            && matchesGovernedCaseRoutingReplay(snapshot?.crystallizationRoutingReceipt, {
                draftPostId,
                draftVersion: workflow.currentSnapshotVersion,
                actorUserId: input.actorUserId,
            ...
            && snapshot.crystallizationRoutingReceipt.caseId === lockingCase.id
        ) {
            return resolveDraftLifecycleReadModel(prisma, { draftPostId, now: input.now });
        }
        throw new DraftWorkflowStateError(
    ...
    const lifecycle = await resolveDraftLifecycleReadModel(prisma, { draftPostId, now });
    if (lifecycle.documentStatus !== 'review') {
        throw new DraftWorkflowStateError(
            'draft_not_in_review',
            409,
            'governed crystallization routing requires a Draft in review',
        );
    }
    if (!lifecycle.circleId || !lifecycle.stableSnapshot.contentHash) {
        throw new DraftWorkflowStateError(
            'draft_version_snapshot_unavailable',
            409,
            'governed crystallization routing requires a circle-bound stable Draft snapshot',
        );
    }
    if (lifecycle.policyProfileDigest !== input.policyProfileDigest) {
        throw new DraftWorkflowStateError(
            'policy_profile_digest_stale',
            409,
            'the submitted policy profile digest is not the current Circle policy',
    ...
    const existingReceipt = lifecycle.stableSnapshot.crystallizationRoutingReceipt;
    if (existingReceipt) {
        if (matchesGovernedCaseRoutingReplay(existingReceipt, {
            draftPostId,
    ...
    await (prisma as any).$transaction(async (tx: any) => {
        const resolution = await resolveRevisionDirectionCrystallizationRoute(tx, {
        ...
        const [workflow, snapshot, governanceCase] = await Promise.all([
            tx.draftWorkflowState.findUnique({ where: { draftPostId } }),
            tx.draftVersionSnapshot.findUnique({
                where: {
            ...
            tx.governanceCase.findUnique({ where: { id: resolution.caseId } }),
        ]);
        if (
            workflow?.documentStatus !== 'review'
            || workflow.currentSnapshotVersion !== lifecycle.stableSnapshot.draftVersion
            || snapshot?.contentHash !== lifecycle.stableSnapshot.contentHash
            || !governanceCase
            || !Number.isInteger(governanceCase.caseVersion)
        ) {
            throw new DraftWorkflowStateError(
                'draft_crystallization_routing_stale',
                409,
                'the Draft, Case, or stable snapshot changed before routing completed',
            );
        }
        await bindGovernanceCaseBrief(tx, {
            caseId: resolution.caseId,
            draftPostId,
            actorPubkey: input.actorPubkey,
            canManage: false,
            idempotencyKey: `crystallization-route:${resolution.requestId}`,
            expectedCaseVersion: governanceCase.caseVersion,
            nativeRequestId: resolution.requestId,
            expectedDraftVersion: lifecycle.stableSnapshot.draftVersion,
            expectedSnapshotDigest: lifecycle.stableSnapshot.contentHash,
            now,
        });
        await recordGovernedCaseCrystallizationRoutingReceipt(tx, {
            draftPostId,
            draftVersion: lifecycle.stableSnapshot.draftVersion,
            actorUserId: input.actorUserId,
            policyProfileDigest: input.policyProfileDigest,
        ...
        await recordGovernedDraftCrystallizationAuthorization(tx, {
            draftPostId,
            draftVersion: lifecycle.stableSnapshot.draftVersion,
            policyProfileDigest: input.policyProfileDigest,
            anchorSignature: requireAnchorSignature(input.anchorSignature),
    ...
    return resolveDraftLifecycleReadModel(prisma, { draftPostId, now });
}
