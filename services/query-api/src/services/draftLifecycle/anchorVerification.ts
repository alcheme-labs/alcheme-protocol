...
async function fetchTransactionFromRpc(signature: string): Promise<{
    tx: any | null;
    reason?: string;
}> {
    const rpcUrl = process.env.SOLANA_RPC_URL || process.env.RPC_URL;
    if (!rpcUrl) return { tx: null, reason: 'rpc_url_unconfigured' };
...
async function verifyDraftLifecycleMilestoneAnchor(input: {
    ...
    const txLookup = await fetchTransactionFromRpc(anchorSignature);
    if (txLookup.reason) return { ok: false, reason: txLookup.reason };
    const tx = txLookup.tx;
