...
import {
    ...
    recordOrdinaryCrystallizationRoutingReceipt,
    updateDraftVersionSnapshotSourceEvidence,
} from './versionSnapshots';
import { lockDraftWorkingCopyForSnapshot } from './paragraphStructure';

type PrismaLike = PrismaClient | Prisma.TransactionClient;

async function inDraftWorkflowTransaction<T>(
    prisma: PrismaLike,
    action: (tx: Prisma.TransactionClient) => Promise<T>,
): Promise<T> {
    const transaction = (prisma as PrismaClient).$transaction;
    return typeof transaction === 'function'
        ? transaction.call(prisma, action) as Promise<T>
        : action(prisma as Prisma.TransactionClient);
}
...
async function transitionDraftWorkflowStateToDrafting(
    ...
    input: {
        ...
        blockingCasePhases?: readonly string[];
    },
): Promise<boolean> {
    return inDraftWorkflowTransaction(prisma, async (tx) => {
        // Review-scoped paragraph references take the same Post lock before
        // they are created. Lock and state mutation must share this transaction.
        await lockDraftWorkingCopyForSnapshot(tx, input.draftPostId);
        if (
            input.blockingCasePhases
            && await findBriefLockingGovernanceCase(tx, input.draftPostId, input.blockingCasePhases)
        ) {
            throw new DraftWorkflowStateError(
                'draft_brief_locked_by_governance_case',
                409,
                'draft is locked as a case brief and cannot return to drafting',
            );
        }
        const updatedAt = sqlTimestampWithoutTimeZone(new Date());
    ...
    return inDraftWorkflowTransaction(prisma, async (tx) => {
        ...
        if (
            ...
            throw new DraftWorkflowStateError(
                'draft_brief_locked_by_governance_case',
                409,
                'draft is locked as an in-flight case brief and cannot be archived',
            );
        }
    ...
    return inDraftWorkflowTransaction(prisma, async (tx) => {
        ...
        if (
            ...
            throw new DraftWorkflowStateError(
                'draft_brief_locked_by_governance_case',
                409,
                'draft is locked as an in-flight case brief and cannot enter crystallization',
            );
        }
...
export async function recordGovernedDraftCrystallizationAuthorization(
    prisma: PrismaLike,
    input: {
        draftPostId: number;
        draftVersion: number;
    ...
    const updated = await prisma.$executeRaw(Prisma.sql`
        ...
        SET
            ...
            updated_at = ${updatedAt}
        WHERE draft_post_id = ${input.draftPostId}
          AND current_snapshot_version = ${input.draftVersion}
          AND document_status = 'review'
          AND (
              crystallization_policy_profile_digest IS NULL
              OR crystallization_policy_profile_digest = ${input.policyProfileDigest}
          )
          AND (
              crystallization_anchor_signature IS NULL
              OR crystallization_anchor_signature = ${input.anchorSignature}
          )
    `);
    if (Number(updated) > 0) return;
    const current = await loadDraftWorkflowStateRow(prisma, input.draftPostId);
    if (
        current
        && normalizeDocumentStatus(current.documentStatus) === 'review'
        && current.currentSnapshotVersion === input.draftVersion
        && current.crystallizationPolicyProfileDigest === input.policyProfileDigest
        && current.crystallizationAnchorSignature === input.anchorSignature
    ) return;
    throw new DraftWorkflowStateError(
        'governed_crystallization_authorization_conflict',
        409,
        'the governed Draft crystallization authorization is stale or conflicting',
    );
}

export async function activateGovernedDraftCrystallization(
    prisma: PrismaLike,
    input: {
        draftPostId: number;
        draftVersion: number;
    ...
    await inDraftWorkflowTransaction(prisma, async (tx) => {
        await lockDraftWorkingCopyForSnapshot(tx, input.draftPostId);
        const current = await loadDraftWorkflowStateRow(tx, input.draftPostId);
        if (
            !current
            || current.currentSnapshotVersion !== input.draftVersion
            || current.crystallizationPolicyProfileDigest !== input.policyProfileDigest
            || !parseNonEmptyString(current.crystallizationAnchorSignature)
        ) {
            throw new DraftWorkflowStateError(
                'governed_crystallization_authorization_missing',
                409,
                'accepted governed crystallization requires the current signed Draft authorization',
            );
        }
        if (normalizeDocumentStatus(current.documentStatus) === 'crystallization_active') return;
        if (normalizeDocumentStatus(current.documentStatus) !== 'review') {
            throw new DraftWorkflowStateError(
                'governed_crystallization_draft_not_current',
                409,
                'accepted governed crystallization requires the same Draft to remain in review',
            );
        }
        const activated = await transitionDraftWorkflowStateToCrystallization(tx, {
            draftPostId: input.draftPostId,
            crystallizationStartedAt: input.now,
            actorUserId: input.actorUserId,
            policyProfileDigest: input.policyProfileDigest,
            anchorSignature: current.crystallizationAnchorSignature!,
        });
        if (!activated) {
            throw new DraftWorkflowStateError(
                'governed_crystallization_activation_conflict',
                409,
                'the governed Draft changed before accepted crystallization activation',
            );
        }
    });
}
...
async function materializeReviewSnapshotAndEnterState(
    ...
    // Paragraph-structure mutations take the same Post row lock. The review
    // snapshot therefore reads either the complete pre-mutation or complete
    // post-mutation working copy, never an interleaving of the two workflows.
    await lockDraftWorkingCopyForSnapshot(prisma, input.row.draftPostId);
...
export const BRIEF_ACTIVE_REVIEW_PHASES = [
    'evidence_review',
    'ready_for_decision',
...
export const BRIEF_CONSUMED_PHASES = [
    ...BRIEF_ACTIVE_REVIEW_PHASES,
    'closed',
    'archived',
] as const;

export const BRIEF_OPEN_PHASES = [
    'intake',
    'proposal_drafting',
    ...BRIEF_ACTIVE_REVIEW_PHASES,
] as const;

export const BRIEF_LOCK_PHASES = [
    ...BRIEF_OPEN_PHASES,
    'closed',
    'archived',
] as const;

export async function findBriefLockingGovernanceCase(
    prisma: PrismaLike,
    draftPostId: number,
    phases: readonly string[],
): Promise<{ id: string; casePhase: string } | null> {
    const row = await prisma.governanceCase.findFirst({
        where: {
            briefDraftPostId: draftPostId,
            casePhase: { in: [...phases] },
        },
        select: { id: true, casePhase: true },
    });
    const casePhase = String(row?.casePhase || '');
    if (!row || !phases.includes(casePhase)) return null;
    return { id: String(row.id), casePhase };
}

export function projectBriefLock(row: { id: string; casePhase: string } | null) {
    if (!row) return null;
    const active = (BRIEF_ACTIVE_REVIEW_PHASES as readonly string[]).includes(row.casePhase);
    const consumed = (BRIEF_CONSUMED_PHASES as readonly string[]).includes(row.casePhase);
    const open = (BRIEF_OPEN_PHASES as readonly string[]).includes(row.casePhase);
    return {
        caseId: row.id,
        casePhase: row.casePhase,
        canReturnToDrafting: !consumed,
        canArchive: !active,
        canCrystallize: !open,
    };
}
...
export async function advanceDraftLifecycleFromReview(
    ...
    if (await findBriefLockingGovernanceCase(prisma, input.draftPostId, BRIEF_CONSUMED_PHASES)) {
        throw new DraftWorkflowStateError(
            'draft_brief_locked_by_governance_case',
            409,
            'draft is locked as a case brief and cannot return to drafting',
    ...
    const updated = await transitionDraftWorkflowStateToDrafting(prisma, {
        ...
        actorUserId: input.actorUserId,
        blockingCasePhases: BRIEF_CONSUMED_PHASES,
    });
    if (!updated) {
        const rowAfterNoop = await loadDraftWorkflowStateRow(prisma, input.draftPostId);
        if (rowAfterNoop) {
...
export async function enterDraftLifecycleCrystallization(
    ...
    if (
        input.routingConfirmation?.path !== 'ordinary_knowledge'
        || input.routingConfirmation?.actionIntent !== 'none'
        || input.routingConfirmation?.humanConfirmed !== true
    ) {
        throw new DraftWorkflowStateError(
            'crystallization_routing_confirmation_required',
            409,
            'ordinary crystallization requires an explicit no-action-intent confirmation',
        );
    }
    const template = input.template || buildDefaultLifecycleTemplate();
    const now = input.now || new Date();
    ...
    if (await findBriefLockingGovernanceCase(prisma, input.draftPostId, BRIEF_OPEN_PHASES)) {
        throw new DraftWorkflowStateError(
            'draft_brief_locked_by_governance_case',
            409,
            'draft is locked as an in-flight case brief and cannot enter crystallization',
    ...
    if (state.documentStatus === 'crystallization_active') {
        ...
        if (!snapshot) {
            throw new DraftWorkflowStateError(
                'draft_version_snapshot_unavailable',
                409,
                'active crystallization has no stable Draft snapshot',
            );
        }
    } else {
    ...
    let updated = false;
    try {
        updated = await inDraftWorkflowTransaction(prisma, async (tx) => {
            // The route receipt and lifecycle transition are one decision. Holding
            // the same Post lock as Case Brief binding prevents either path from
            // committing after it observed the other path as absent.
            await lockDraftWorkingCopyForSnapshot(tx, input.draftPostId);
            if (await findBriefLockingGovernanceCase(tx, input.draftPostId, BRIEF_OPEN_PHASES)) {
                throw new DraftWorkflowStateError(
                    'draft_brief_locked_by_governance_case',
                    409,
                    'draft is locked as an in-flight case brief and cannot enter crystallization',
            ...
            await recordOrdinaryCrystallizationRoutingReceipt(tx, {
                draftPostId: input.draftPostId,
                draftVersion: state.currentSnapshotVersion,
                actorUserId: input.actorUserId,
                policyProfileDigest: input.policyProfileDigest,
                evaluatedAt: now,
                allowRetryActorSupersession: input.allowRetryActorSupersession,
            });
            if (state.documentStatus === 'crystallization_active') return true;
            return transitionDraftWorkflowStateToCrystallization(tx, {
                draftPostId: input.draftPostId,
        ...
        if (error instanceof Error && error.message === 'draft_crystallization_routing_conflict') {
            throw new DraftWorkflowStateError(
                'draft_crystallization_routing_conflict',
                409,
                'this Draft snapshot already has a different crystallization route',
            );
        }
        throw error;
    }

    if (state.documentStatus === 'crystallization_active') return state;
    if (!updated) {
        const rowAfterNoop = await loadDraftWorkflowStateRow(prisma, input.draftPostId);
        if (rowAfterNoop) {
            const nextState = mapRowToView(rowAfterNoop);
...
export async function retryDraftLifecycleCrystallization(
    ...
    const current = await resolveDraftWorkflowState(prisma, {
        draftPostId: input.draftPostId,
        circleId: input.circleId,
        template: input.template || buildDefaultLifecycleTemplate(),
        seedStartedAt: input.seedStartedAt,
        now: input.now || new Date(),
    });
    if (current.documentStatus === 'crystallization_failed') {
        const snapshot = await loadDraftVersionSnapshot(prisma, {
            draftPostId: input.draftPostId,
            draftVersion: current.currentSnapshotVersion,
        });
        const receipt = snapshot?.crystallizationRoutingReceipt;
        if (receipt?.path === 'governed_case') {
            if (
                receipt.draftPostId !== input.draftPostId
                || receipt.draftVersion !== current.currentSnapshotVersion
                || receipt.snapshotContentHash !== snapshot?.contentHash
                || receipt.policyProfileDigest !== input.policyProfileDigest
            ) {
                throw new DraftWorkflowStateError(
                    'governed_crystallization_retry_context_mismatch',
                    409,
                    'governed crystallization retry must preserve the accepted frozen route',
                );
            }
            const reactivated = await transitionDraftWorkflowStateToCrystallization(prisma, {
                draftPostId: input.draftPostId,
                crystallizationStartedAt: input.now || new Date(),
                actorUserId: input.actorUserId,
                policyProfileDigest: input.policyProfileDigest,
                anchorSignature: input.anchorSignature,
                allowRetryFromFailure: true,
                blockingCasePhases: BRIEF_OPEN_PHASES,
            });
            if (!reactivated) {
                throw new DraftWorkflowStateError(
                    'governed_crystallization_retry_conflict',
                    409,
                    'the governed Draft changed before crystallization retry',
                );
            }
            const row = await loadDraftWorkflowStateRow(prisma, input.draftPostId);
            if (!row) {
                throw new DraftWorkflowStateError(
                    'draft_workflow_state_unavailable',
                    500,
                    'draft workflow state could not be reloaded after governed retry',
                );
            }
            return mapRowToView(row);
        }
    }
    return enterDraftLifecycleCrystallization(prisma, {
        ...input,
        allowRetryActorSupersession: current.documentStatus === 'crystallization_failed',
        routingConfirmation: {
            path: 'ordinary_knowledge',
            actionIntent: 'none',
            humanConfirmed: true,
...
export async function archiveDraftLifecycle(
    ...
    if (await findBriefLockingGovernanceCase(prisma, input.draftPostId, BRIEF_ACTIVE_REVIEW_PHASES)) {
        throw new DraftWorkflowStateError(
            'draft_brief_locked_by_governance_case',
            409,
            'draft is locked as an in-flight case brief and cannot be archived',
    ...
    const updated = await transitionDraftWorkflowStateToArchived(prisma, {
        ...
        archivedAt: now,
        actorUserId: input.actorUserId,
        blockingCasePhases: BRIEF_ACTIVE_REVIEW_PHASES,
    });
    if (!updated) {
        const rowAfterNoop = await loadDraftWorkflowStateRow(prisma, input.draftPostId);
        if (rowAfterNoop) {
