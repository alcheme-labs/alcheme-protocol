...
import { isDiscussionAnalysisProviderError, toDiscussionAnalysisProviderError } from './providerErrors';
...
export async function runDiscussionMessageAnalyzeJob(input: {
    ...
    try {
        ...
        await runDiscussionAnalysisPostWriteEffects({
            ...
            nextStatus: 'ready',
            nextFocusLabel: analysis.focusLabel,
            requestedByUserId: input.requestedByUserId ?? null,
        });
        ...
        return {
            ...
            method: analysis.relevanceMethod || 'unknown',
            isFeatured: analysis.isFeatured,
        };
    } catch (error) {
        if (isDiscussionAnalysisProviderError(error)) {
            ...
            const rows = await input.prisma.$transaction(async (tx) => {
                const updatedRows = await tx.$queryRaw<Array<{ envelopeId: string }>>(Prisma.sql`
                    ...
                    SET
                        ...
                        analysis_error_code = ${providerError.code},
                        analysis_error_message = ${providerError.message.slice(0, 512)},
                        updated_at = NOW()
                    WHERE envelope_id = ${input.envelopeId}
                    RETURNING envelope_id AS "envelopeId"
                `);
            ...
            try {
                ...
                // best effort only; provider error must still be retried
            }
        ...
        await runDiscussionAnalysisPostWriteEffects({
            ...
            previousStatus: message.relevanceStatus,
            nextStatus: 'failed',
            nextFocusLabel: message.focusLabel,
            requestedByUserId: input.requestedByUserId ?? null,
        });
