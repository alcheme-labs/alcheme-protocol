...
export function inferSemanticFacets(input: {
    ...
    if (
        ...
        || /\b(compare|comparison|evaluate|evaluation|weigh|weighing)\b[^.!?]{0,160}\b(costs?|privacy|boundaries|recovery|time|options?|criteria|trade-?offs?|risks?)\b/i.test(text)
    ) {
        facets.add('criteria');
    }
    ...
    if (
        /(建议|应该|可以先|不如|我们需要|最好|要不要|按这个思路|先产出|试试看|先生成|起一个草稿|先做|先试)/i.test(text)
        || /\b(we should|should we|we need(?: to)?|let(?:'|’)s|i suggest|i recommend|before choosing)\b/i.test(text)
    ) {
        facets.add('proposal');
    }
