...
async function loadDraftThreadStats(
    ...
    const rows = await prisma.$queryRaw<SummaryThreadStatsRow[]>(Prisma.sql`
        SELECT
            COUNT(*) FILTER (WHERE state IN ('open', 'proposed', 'accepted'))::int AS "openThreadCount",
...
function buildCircleSummaryLlmPrompt(input: {
    ...
    const messageLines = input.recentMessages
        .slice(-8)
        .map((row) => `[${row.createdAt.toISOString()}] ${row.senderHandle || localizeQueryApiCopy('identity.genericMember', input.locale)}: ${(row.payloadText || '').trim()}`);
