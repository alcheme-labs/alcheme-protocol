...
type ExternalAppRegistrationData = Prisma.ExternalAppCreateInput;

export async function registerExternalApp(
  prisma: Pick<PrismaClient, "externalApp">,
  input: RegisterExternalAppInput,
): Promise<ExternalApp> {
  const data = buildExternalAppRegistrationData(input);
  return prisma.externalApp.upsert({
    ...
    update: {
      ...
      claimAuthMode: data.claimAuthMode,
...
export async function createExternalApp(
  ...
  return prisma.externalApp.create({
    data: buildExternalAppRegistrationData(input),
  });
}

function buildExternalAppRegistrationData(
  input: RegisterExternalAppInput,
): ExternalAppRegistrationData {
...
function assertSolanaPublicKey(value: string, errorCode: string): void {
  try {
    new PublicKey(value);
  } catch {
