...
import { markSourceMaterialsUsedInDraft } from '../services/sourceMaterials/lifecycleHooks';
...
export async function generateGhostDraft(
    ...
    await markSourceMaterialsUsedInDraft(prisma, {
        draftPostId: postId,
        actorUserId: userId,
        sourceMaterialIds: normalizeSourceMaterialIds(options?.sourceMaterialIds ?? null),
    });
...
async function buildContext(
    ...
    const post = await prisma.post.findUnique({
        where: { id: postId, safetyQuarantined: false },
        include: {
    ...
    const sourceMaterialContext = post.circle?.id
        ...
        : [];
    const requestedSourceMaterialIds = normalizeSourceMaterialIds(options?.sourceMaterialIds ?? null);
    if (requestedSourceMaterialIds.length > 0) {
        const groundedIds = new Set(sourceMaterialContext.map((item) => Number(item.materialId)));
        const missingIds = requestedSourceMaterialIds.filter((id) => !groundedIds.has(id));
