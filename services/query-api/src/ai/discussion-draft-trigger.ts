...
} from '../services/discussion/discussionMessageKinds';
...
} from '../services/discussion/draftSourceConsumption';
...
export const DISCUSSION_DRAFT_TRIGGER_EXCLUDED_MESSAGE_KINDS = [
...
export async function createTriggeredDraftPostWithInitialSnapshot(
    ...
    return prisma.$transaction(async (tx) => {
        if (input.sourceMessageIds?.length) {
            const sourceRows = await tx.$queryRaw<Array<{ envelopeId: string }>>(Prisma.sql`
                SELECT envelope_id AS "envelopeId"
                FROM circle_discussion_messages
                WHERE circle_id = ${input.circleId}
                  AND envelope_id IN (${Prisma.join(input.sourceMessageIds)})
                  AND deleted = FALSE
                  AND tombstoned_at IS NULL
                  AND is_ephemeral = FALSE
                  AND COALESCE(relevance_status, 'ready') = 'ready'
                  AND COALESCE(focus_label, 'contextual') <> 'off_topic'
                  AND COALESCE(message_kind, 'plain') NOT IN (${Prisma.join([...DISCUSSION_DRAFT_TRIGGER_EXCLUDED_MESSAGE_KINDS])})
                FOR SHARE
            `);
            if (new Set(sourceRows.map((row) => row.envelopeId)).size !== new Set(input.sourceMessageIds).size) {
                throw new DiscussionInitialDraftError({
                    code: 'auto_draft_source_no_longer_eligible',
                    message: 'automatic draft source eligibility changed before persistence',
                    retryable: false,
                });
            }
        }
...
export function isDraftTriggerEligibleMessageKind(messageKind: string | null | undefined): boolean {
    const normalized = normalizeDiscussionMessageKind(messageKind);
    if (!normalized) return true;
    if (isDiscussionForwardProjectionMessageKind(normalized)) return false;
    return !DISCUSSION_DRAFT_TRIGGER_EXCLUDED_MESSAGE_KINDS.includes(
        normalized as typeof DISCUSSION_DRAFT_TRIGGER_EXCLUDED_MESSAGE_KINDS[number],
    );
}
...
export async function maybeTriggerGhostDraftFromDiscussion(input: {
    ...
    let activeSourceClaim: { sourceMessagesDigest: string; claimToken: string } | null = null;
    let sourceClaimCompleted = false;
    ...
    try {
        ...
        const allWindowMessages = ordered
            .map((row) => ({
                ...row,
        ...
        const sourceState = await loadDiscussionDraftSourceState(input.prisma, input.circleId);
        const lastAutoSourceToLamport = sourceState?.lastAutoSourceToLamport === null
            || sourceState?.lastAutoSourceToLamport === undefined
            ? BigInt(0)
            : BigInt(sourceState.lastAutoSourceToLamport);
        const consumedEnvelopeIds = new Set(await listConsumedDiscussionDraftSourceEnvelopeIds(input.prisma, {
            circleId: input.circleId,
            envelopeIds: allWindowMessages.map((row) => row.envelopeId),
        }));
        const sourceMessages = allWindowMessages.filter((row) =>
            row.lamport > lastAutoSourceToLamport
            && !consumedEnvelopeIds.has(row.envelopeId)
        );
        const sourceMessageIds = sourceMessages.map((row) => row.envelopeId);
        const sourceDigest = computeDiscussionDraftSourceDigest(sourceMessageIds);
        const windowMetadata = {
            windowEnvelopeIds: allWindowMessages.map((row) => row.envelopeId),
            sourceEnvelopeIds: sourceMessageIds,
            consumedEnvelopeIds: Array.from(consumedEnvelopeIds),
            lastAutoSourceToLamport: lastAutoSourceToLamport.toString(),
            windowDigest: buildTriggerWindowDigest(allWindowMessages),
            sourceDigest,
        };

        if (sourceMessages.length < triggerConfig.minMessages) {
            const reason = sourceState ? 'insufficient_new_source_messages' : 'insufficient_messages';
            await writeGhostRunAudit(input.prisma, {
                ...auditBase,
                status: 'skipped',
                reason,
        ...
        const focusStats = computeFocusStats(
            sourceMessages.map((row) => row.focusScore),
            0.35,
        );
        const focusedCount = focusStats.focusedCount;
        const focusedRatio = focusStats.focusedRatio;
        const participantCount = new Set(sourceMessages.map((row) => row.senderPubkey)).size;
        const spamCount = sourceMessages.filter((row) => row.spamScore >= 0.6).length;
        const spamRatio = spamCount / Math.max(1, sourceMessages.length);
        const questionCount = sourceMessages.filter((row) =>
        ...
        const summary = await summarizeDiscussionThread({
            ...
            messages: sourceMessages.map((row) => ({
        ...
        if (triggerDecision.recommendedAction === 'notify_only') {
            const notification = await notifyDraftOpportunity({
                ...
                senderPubkeys: sourceMessages.map((m) => m.senderPubkey),
                summary: summary.summary,
            ...
            const sourceSemanticFacets = collectSourceSemanticFacets(sourceMessages);
            const sourceAuthorAnnotations = collectSourceAuthorAnnotations(sourceMessages);
            try {
        ...
        const sourceClaim = await claimDiscussionDraftSource(input.prisma, {
            circleId: input.circleId,
            sourceMessagesDigest: sourceDigest,
            sourceMessageIds,
            sourceKind: 'auto_draft',
            sourceFromLamport: sourceMessages[0]?.lamport ?? null,
            sourceToLamport: sourceMessages[sourceMessages.length - 1]?.lamport ?? null,
        ...
        if (sourceClaim.status === 'succeeded') {
            await writeGhostRunAudit(input.prisma, {
                ...
                focusedRatio,
                questionCount,
                draftPostId: sourceClaim.draftPostId,
                summaryMethod: summary.method,
                summaryPreview: truncateSummaryPreview(summary.summary),
                metadata: {
                    ...windowMetadata,
        ...
        if (sourceClaim.status === 'pending') {
            await writeGhostRunAudit(input.prisma, {
                ...auditBase,
                status: 'skipped',
                reason: 'source_generation_in_progress',
                messageCount: sourceMessages.length,
                focusedCount,
                focusedRatio,
                questionCount,
                ...
                metadata: {
                    ...
                    topicHeat,
                    judge: triggerDecision,
                    claimedUntil: sourceClaim.claimedUntil?.toISOString() ?? null,
                },
            });
            return { triggered: false, reason: 'source_generation_in_progress' };
        }
        ...
        const initialDraft = await generateInitialDiscussionDraft(input.prisma, {
            ...
            if (activeSourceClaim && !sourceClaimCompleted) {
                try {
                    await failDiscussionDraftSourceClaim(input.prisma, {
                        circleId: input.circleId,
                        sourceMessagesDigest: activeSourceClaim.sourceMessagesDigest,
                        claimToken: activeSourceClaim.claimToken,
                        failureReason: generationError.code,
                    });
                } catch (claimError) {
                    const message = claimError instanceof Error ? claimError.message : String(claimError);
                    console.warn(`discussion draft trigger: failed to mark source claim failed (${message})`);
                }
            }
        ...
        if (activeSourceClaim) {
            await completeDiscussionDraftSourceClaim(input.prisma, {
                circleId: input.circleId,
                sourceMessagesDigest: activeSourceClaim.sourceMessagesDigest,
                claimToken: activeSourceClaim.claimToken,
                draftPostId: draftPost.id,
                sourceKind: 'auto_draft',
                sourceMessages: sourceMessages.map((row) => ({
                    envelopeId: row.envelopeId,
                    lamport: row.lamport,
                })),
                advanceAutoCursor: true,
                autoSourceFromLamport: sourceMessages[0]?.lamport ?? null,
                autoSourceToLamport: sourceMessages[sourceMessages.length - 1]?.lamport ?? null,
            });
            sourceClaimCompleted = true;
        }
        ...
        if (activeSourceClaim && !sourceClaimCompleted) {
            try {
                ...
                console.warn(`discussion draft trigger: failed to mark source claim failed (${claimMessage})`);
            }
        }
        await writeGhostRunAudit(input.prisma, {
            ...auditBase,
