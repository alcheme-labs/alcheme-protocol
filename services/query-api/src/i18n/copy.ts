...
const ROLE_LABELS: Record<GovernanceRole | 'higher', LocalizedCopy> = {
    ...
    Elder: { en: 'Senior Contributor', zh: '资深贡献者' },
    Member: { en: 'Contributor', zh: '贡献者' },
    Initiate: { en: 'Participant', zh: '参与者' },
...
export type QueryApiCopyKey =
    ...
    | 'draft.scopedEdit.circleRequired'
    | 'draft.scopedEdit.scopeRequired'
    | 'draft.scopedEdit.draftingOnly'
    | 'draft.scopedEdit.permissionRequired'
    | 'draft.scopedEdit.workingCopyConflict'
    | 'draft.scopedEdit.circleMismatch'
    | 'draft.scopedEdit.invalidScope'
    | 'draft.scopedEdit.paragraphCountChanged'
    | 'draft.scopedEdit.paragraphEmpty'
    | 'draft.scopedEdit.scopeExceeded'
    | 'draft.discussion.reviewStageRequired'
    | 'draft.discussion.stableSnapshotRequired'
    | 'draft.content.draftingOnly'
    | 'draft.content.workingCopyPreconditionRequired'
...
const COPY: Record<QueryApiCopyKey, LocalizedCopy> = {
    ...
    'draft.scopedEdit.circleRequired': {
        en: 'Paragraph editing requires a circle-bound draft.',
        zh: '段落编辑需要草稿已绑定圈层。',
    },
    'draft.scopedEdit.scopeRequired': {
        en: 'Limited draft editors must choose a paragraph before saving.',
        zh: '受限草稿编辑者保存前必须选择当前段落。',
    },
    'draft.scopedEdit.draftingOnly': {
        en: 'Paragraph editing is only available while the draft is still being drafted.',
        zh: '段落编辑只在草稿修订阶段可用。',
    },
    'draft.scopedEdit.permissionRequired': {
        en: 'You need source participant access or an active temporary edit grant to edit this paragraph.',
        zh: '需要来源参与者权限或有效临时编辑权限，才能编辑当前段落。',
    },
    'draft.scopedEdit.workingCopyConflict': {
        en: 'Draft content changed. Refresh before saving.',
        zh: '草稿内容已变化，请刷新后重试。',
    },
    'draft.scopedEdit.circleMismatch': {
        en: 'Draft circle context changed. Refresh before saving.',
        zh: '草稿圈层上下文已变化，请刷新后重试。',
    },
    'draft.scopedEdit.invalidScope': {
        en: 'This paragraph edit scope is invalid.',
        zh: '当前段落编辑范围无效。',
    },
    'draft.scopedEdit.paragraphCountChanged': {
        en: 'Paragraph-scoped edits cannot add or remove paragraphs.',
        zh: '段落级编辑不能新增或删除段落。',
    },
    'draft.scopedEdit.paragraphEmpty': {
        en: 'The target paragraph cannot be empty.',
        zh: '当前段落不能为空。',
    },
    'draft.scopedEdit.scopeExceeded': {
        en: 'This change is outside the current paragraph.',
        zh: '本次修改超出了当前段落范围。',
    },
    'draft.discussion.reviewStageRequired': {
        en: 'Issues can only be opened or replied to while the draft is in review.',
        zh: '只有草稿处于评审阶段时，才能提出或回复评审问题。',
    },
    'draft.discussion.stableSnapshotRequired': {
        en: 'This draft does not have a stable review snapshot yet.',
        zh: '当前草稿还没有可评审的稳定版本快照。',
    },
    'draft.content.draftingOnly': {
        en: 'Draft content can only be edited while the draft is still being drafted.',
        zh: '草稿正文只能在草稿修订阶段修改。',
    },
    'draft.content.workingCopyPreconditionRequired': {
        en: 'Refresh the draft before saving so the current working copy version can be verified.',
        zh: '保存前请先刷新草稿，以确认当前 working copy 版本。',
    },
    'discussion.forward.emptySourceMessage': {
    ...
    'announcement.notificationConfirmTitle': {
        en: 'Announcement needs confirmation',
        zh: '有公告需要确认',
        es: 'Anuncio pendiente de confirmación',
        fr: 'Annonce à confirmer',
    },
    'identity.genericMember': {
        en: 'A member',
        zh: '某位成员',
