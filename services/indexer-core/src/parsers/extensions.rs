...
const CONTRIBUTION_SCORE_UPDATED_MARKER: &str = "Contribution score updated:";
...
impl ExtensionLogParser for ContributionEngineLogParser {
    ...
    fn parse_logs(&self, logs: &[String]) -> Vec<ParsedExtensionEvent> {
        ...
        for log in logs {
            ...
            if log.contains(CONTRIBUTION_SCORE_UPDATED_MARKER) {
                let contributor =
                    extract_field(log, "contributor=").unwrap_or_else(|| "unknown".to_string());
                let new_weight =
                    extract_field(log, "new_weight=").unwrap_or_else(|| "unknown".to_string());

                events.push(ParsedExtensionEvent::ContributionEngine(
                    ContributionEngineEvent::ContributionScoreUpdated {
                        contributor,
                        new_weight,
                    },
                ));
...
mod tests {
    ...
    fn parses_reference_added_log_into_contribution_engine_event() {
        ...
        let logs =
            vec!["Program log: Reference added: Source111 -> Target222 type=Citation".to_string()];
    ...
    #[test]
    fn parses_reserved_contribution_score_update_without_projecting_it() {
        let registry = ExtensionParserRegistry::default();
        let logs = vec![
            "Program log: Contribution score updated: contributor=Contributor111, new_weight=42"
                .to_string(),
        ];
        ...
        match &events[0] {
            ParsedExtensionEvent::ContributionEngine(
                ContributionEngineEvent::ContributionScoreUpdated {
                    contributor,
                    new_weight,
                },
            ) => {
                assert_eq!(contributor, "Contributor111");
                assert_eq!(new_weight, "42");
            }
            other => panic!("expected ContributionScoreUpdated event, got {:?}", other),
        }
    }
