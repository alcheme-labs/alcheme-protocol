...
use crate::database::db_writer::{ContentInteractionEventContext, DbWriter, ProjectedUserProfile};
use crate::parsers::extensions::{
    ContributionEngineEvent, ExtensionParserRegistry, ParsedExtensionEvent,
};
use crate::rpc_http::{build_rpc_http_client, RpcHttpClient};
...
const CIRCLE_MEMBER_ACCOUNT_DISCRIMINATOR: [u8; 8] = [31, 9, 237, 222, 220, 6, 92, 154];
const CIRCLE_MEMBER_ACCOUNT_SIZE: usize = 60;
const CIRCLE_MEMBER_ACCOUNT_DISCRIMINATOR_BASE58: &str = "6C7gD7CHMQd";
...
impl EventParser {
    ...
    pub async fn reconcile_circle_membership_account_snapshots(&self) -> anyhow::Result<usize> {
        let Some(program_id) = self.circle_manager_program_id else {
            return Ok(0);
        };
        let accounts = self
            .rpc_client
            .get_program_accounts(
                &program_id,
                CIRCLE_MEMBER_ACCOUNT_SIZE,
                CIRCLE_MEMBER_ACCOUNT_DISCRIMINATOR_BASE58,
            )
            .await?;
        let mut reconciled = 0usize;
        for account in accounts {
            let account_pubkey = account.pubkey.parse::<Pubkey>().map_err(|error| {
                anyhow!(
                    "invalid circle member account {}: {}",
                    account.pubkey,
                    error
                )
            })?;
            let encoded = account.account.data.first().ok_or_else(|| {
                anyhow!(
                    "missing circle member account payload for {}",
                    account_pubkey
                )
            })?;
            let data = base64::engine::general_purpose::STANDARD
                ...
                .map_err(|error| {
                    anyhow!(
                        "failed to decode circle member account {}: {}",
                        account_pubkey,
                        error
                    )
                })?;
            let projection = decode_circle_member_account_snapshot(&account_pubkey, &data)?;
            self.db_writer
    ...
    pub async fn route_event(
        ...
        match event {
            ...
            ProtocolEvent::VerificationAttributeAdded {
                ...
                debug!(
                    "Verification attribute event observed without read-model projection: {} {} by {}",
                    identity_id, attribute_type, verifier
                );
            }
                ...
                self.db_writer
                    .record_interaction(
                        ...
                        ContentInteractionEventContext {
                            tx_signature: projection_context.signature.as_deref(),
                            event_index: projection_context.event_index,
                            slot: projection_context.slot,
                        },
                    )
                    .await?;
            }

            ProtocolEvent::ContentInteractionV2 {
                content_id,
                actor,
                interaction_type,
                timestamp,
            ...
            ProtocolEvent::DraftLifecycleMilestoneV2 {
                ...
                debug!(
                    "Draft lifecycle milestone observed without read-model projection: {} {:?} by {}",
                    draft_post_id, action, actor
                );
            }
            ...
            ProtocolEvent::CircleRestored {
                ...
                self.db_writer
                    .restore_circle(circle_id as i32, timestamp, projection_context.slot)
                    .await?;
            }
            ...
            ProtocolEvent::ProofAttestorRegistered {
                ...
                debug!(
                    "Proof attestor registration observed without read-model projection: {} by {}",
                    attestor, registered_by
                );
            }
            ...
            ProtocolEvent::MembershipAttestorRegistered {
                ...
                debug!(
                    "Membership attestor registration observed without read-model projection: {} by {}",
                    attestor, registered_by
                );
            }
            ...
            ProtocolEvent::MembershipAttestorRevoked {
                ...
                debug!(
                    "Membership attestor revocation observed without read-model projection: {} by {}",
                    attestor, revoked_by
                );
            }
            ...
            ProtocolEvent::PresenceUpdated {
                user_id, status, ..
            } => {
                debug!(
                    "Presence update observed without durable read-model projection: {} {}",
                    user_id, status
                );
            }
            ...
            ProtocolEvent::AccessRuleUpdated {
                user,
                rule_id,
                rule_change,
                ..
                ...
                debug!(
                    "Access rule update observed without read-model projection: {} {} {:?}",
                    user, rule_id, rule_change
                );
            }
            ...
            ProtocolEvent::PermissionDenied {
                ...
                debug!(
                    "Permission denial observed without read-model projection: {} {:?} {}",
                    requester, permission, reason
                );
            }
            ...
            ProtocolEvent::RegistryUpgraded {
                ...
                debug!(
                    "Registry upgrade observed without read-model projection: {} ({} -> {})",
                    registry_id, old_version, new_version
                );
            }
    ...
    pub async fn process_events(
        ...
        for (event_index, event) in events.into_iter().enumerate() {
            ...
            match self.route_event(event, &event_context).await {
                Ok(_) => processed += 1,
                Err(e) => {
    ...
    pub async fn reconcile_knowledge_account_snapshot(
        ...
        ensure_knowledge_snapshot_matches_expected(
...
pub(crate) fn project_circle_membership_event(
    ...
    ProjectedCircleMembership {
        ...
        status,
        on_chain_address: derive_circle_member_on_chain_address(
...
pub(crate) fn project_external_app_registered_v2_event(
    ...
    ProjectedExternalAppRegistryAnchor {
        ...
        record_pda: derive_external_app_record_pda(external_app_registry_program_id, &app_id_hash)
...
fn ensure_knowledge_snapshot_matches_expected(
    expected_knowledge_id: &str,
    knowledge_account_pubkey: &Pubkey,
    snapshot: &KnowledgeAccountSnapshot,
) -> anyhow::Result<()> {
...
pub(crate) fn project_content_anchor_v2_event(
    ...
    let (content_type, reply_to, thread_root, repost_of, reply_depth) = match relation {
        ...
        ContentAnchorRelation::RepostById {
            ...
            None,
            None,
            Some(original_content_id.to_string()),
            0,
        ),
...
impl RpcAccountClient {
    ...
    async fn get_program_accounts(
        ...
        program_id: &Pubkey,
        data_size: usize,
        discriminator_base58: &str,
    ) -> anyhow::Result<Vec<RpcProgramAccount>> {
        let response = self
            .call_raw(
                "getProgramAccounts",
        ...
        if let Some(error) = response.error {
            return Err(anyhow!(
                "RPC method getProgramAccounts failed for {}: code={}, message={}",
                program_id,
                error.code,
                error.message
            ));
        ...
        serde_json::from_value::<Vec<RpcProgramAccount>>(result)
            .map_err(|error| anyhow!("failed to decode getProgramAccounts result: {}", error))
    }
    ...
    async fn call_raw(&self, method: &str, params: Value) -> anyhow::Result<RpcResponse<Value>> {
        ...
        match result {
            Ok((body, response_bytes)) => {
                let outcome = if body.error.is_some() {
                    "rpc_error"
                } else {
                    "success"
                };
                crate::metrics::record_rpc_request(
                    method,
                    "indexer_core",
    ...
    async fn call_raw_inner(
        ...
        method: &str,
        params: Value,
    ) -> anyhow::Result<(RpcResponse<Value>, usize)> {
        let payload = json!({
            "jsonrpc": "2.0",
            "id": 1,
            "method": method,
...
pub(crate) fn decode_circle_member_account_snapshot(
    account_pubkey: &Pubkey,
    data: &[u8],
) -> anyhow::Result<ProjectedCircleMembership> {
    if data.len() != CIRCLE_MEMBER_ACCOUNT_SIZE || data[..8] != CIRCLE_MEMBER_ACCOUNT_DISCRIMINATOR
    {
        return Err(anyhow!(
            "invalid circle member account layout for {}",
            account_pubkey
        ));
    }
    let circle_id = i32::from(data[8]);
    let member = Pubkey::new_from_array(
        data[9..41]
            .try_into()
            .map_err(|_| anyhow!("invalid circle member pubkey for {}", account_pubkey))?,
    );
    let status = match data[41] {
        ...
        value => {
            return Err(anyhow!(
                "invalid circle member status {} for {}",
                value,
                account_pubkey
            ))
        }
    };
    let role = match data[42] {
        0 => "Owner",
        1 => "Admin",
        2 => "Moderator",
        3 => "Member",
        value => {
            return Err(anyhow!(
                "invalid circle member role {} for {}",
                value,
                account_pubkey
            ))
        }
    };
    let changed_at = i64::from_le_bytes(
        data[51..59]
            .try_into()
            .map_err(|_| anyhow!("invalid circle member updated_at for {}", account_pubkey))?,
    );
    Ok(ProjectedCircleMembership {
        circle_id,
        member_pubkey: member.to_string(),
        role,
        status,
        on_chain_address: Some(account_pubkey.to_string()),
        changed_at,
    })
}
...
mod tests {
    ...
    #[test]
    fn knowledge_snapshot_mismatch_error_mentions_expected_and_actual_ids() {
        let knowledge_account = Pubkey::new_unique();
        let snapshot = KnowledgeAccountSnapshot {
            knowledge_id_hex: "actual".to_string(),
            on_chain_address: knowledge_account.to_string(),
            ipfs_cid: "bafy".to_string(),
        };

        let error =
            ensure_knowledge_snapshot_matches_expected("expected", &knowledge_account, &snapshot)
                .expect_err("expected mismatched knowledge snapshot to fail");
        let message = error.to_string();

        assert!(message.contains(&knowledge_account.to_string()));
