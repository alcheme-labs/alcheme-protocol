...
const MAX_RPC_FAILURE_BACKOFF: Duration = Duration::from_secs(30);

fn rpc_failure_backoff(base: Duration, consecutive_failures: u32) -> Duration {
    let exponent = consecutive_failures.saturating_sub(1).min(5);
    let multiplier = 1_u32.checked_shl(exponent).unwrap_or(u32::MAX);
    base.saturating_mul(multiplier).min(MAX_RPC_FAILURE_BACKOFF)
}
...
impl LocalProgramListener {
    ...
    pub async fn start(&mut self) -> Result<()> {
        ...
        loop {
            ...
            let latest_slot = match self.rpc_client.get_slot().await {
                Ok(slot) => slot,
                Err(error) => {
                    consecutive_rpc_failures = consecutive_rpc_failures.saturating_add(1);
                    let retry_delay =
                        rpc_failure_backoff(self.poll_interval, consecutive_rpc_failures);
                    let _ = self
                        .runtime_state_store
                        .mark_error(&format!("local_program_head_slot_failed: {error}"))
                        .await;
                    warn!(
                        "Failed to fetch latest slot for program-cursor listener: {:?}; retrying in {:?}",
                        error,
                        retry_delay
                    );
            ...
            let pages = match self.fetch_program_pages(checkpoint_slot, floor_slot).await {
                ...
                Err(error) => {
                    ...
                    let _ = self
                        ...
                        .await;
                    warn!(
                        "Failed to fetch program-targeted backfill pages: {:?}; retrying in {:?}",
                        error, retry_delay
                    );
                    sleep(retry_delay).await;
                    continue;
            ...
            let overlap_candidates = if self.live_subscriber.take_reconnect_backfill_pending() {
                match self
                    .fetch_overlap_candidates(
                        checkpoint_slot,
                        self.live_subscriber.overlap_signature_limit(),
                    )
                    .await
                {
                    ...
                    Err(error) => {
                        let _ = self
                            ...
                            .await;
                        warn!(
                            "Failed to run overlap backfill after logs reconnect: {:?}",
                            error
                        );
                        Vec::new()
                    }
            ...
            let mut failed_candidate: Option<ProgramSignatureCandidate> = None;
            let mut candidate_rpc_failure = false;
            for (idx, candidate) in candidates.iter().enumerate() {
                let _ = self
                    ...
                    .await;
                let transaction = match self.rpc_client.get_transaction(&candidate.signature).await
                {
                    ...
                    Err(error) => {
                        let _ = self
                            .runtime_state_store
                            .mark_error(&format!(
                                "local_program_fetch_tx_failed[{}]: {}",
                                candidate.signature, error
                            ))
                            .await;
                        warn!(
                            "Failed to fetch program-targeted tx {} at slot {}: {:?}",
                            candidate.signature, candidate.slot, error
                        );
                        failed_candidate = Some(candidate.clone());
                        candidate_rpc_failure = true;
                        break;
                    }
                };
                match transaction {
                    ...
                    None => {
                        let _ = self
                            .runtime_state_store
                            .mark_error(&format!(
                                "local_program_fetch_tx_missing[{}]",
                                candidate.signature
                            ))
                            .await;
                        warn!(
                            "Program-targeted tx {} at slot {} was unavailable; pausing cursor advancement for retry",
                            candidate.signature,
                            candidate.slot
                        );
                        failed_candidate = Some(candidate.clone());
            ...
            if let Some(candidate) = failed_candidate {
                let retry_delay = if candidate_rpc_failure {
                    consecutive_rpc_failures = consecutive_rpc_failures.saturating_add(1);
                    rpc_failure_backoff(self.poll_interval, consecutive_rpc_failures)
                } else {
                    self.poll_interval
                ...
                warn!(
                    "Pausing local program cursor advancement after failed tx {} at slot {}; retrying in {:?}",
                    candidate.signature,
                    candidate.slot,
                    retry_delay
                );
            ...
            if let Some(last) = candidates.last() {
                self.checkpoint_manager.update(last.slot).await?;
            }
            self.advance_program_cursors(&pages, floor_slot).await?;
            if let Some(last) = candidates.last() {
                self.runtime_state_store
                    .mark_slot_completed(last.slot)
                    .await?;
            }
            consecutive_rpc_failures = 0;
        }
    ...
    async fn fetch_program_pages(
        ...
        for program_id in self.current_program_ids() {
            ...
            let oldest = fetched
                .last()
                .map(|(signature, slot)| (signature.clone(), *slot));
    ...
    merge_signature_candidates(
        pages
            ...
            .flat_map(|page| {
                page.signatures
                    .iter()
                    .map(|candidate| (page.program_id.clone(), candidate.clone()))
            })
...
fn merge_signature_candidates(
    candidates: Vec<ProgramSignatureCandidate>,
) -> Vec<ProgramSignatureCandidate> {
...
mod tests {
    ...
    fn local_program_listener_uses_signature_tie_breaker_for_same_slot() {
        let merged = merge_program_signature_pages(&[ProgramSignaturePage {
            program_id: "prog-a".to_string(),
            fetched_count: 2,
    ...
    #[test]
    fn local_program_listener_stops_before_advancing_after_process_failure() {
        let source = include_str!("local_program_listener.rs");
        let runtime_source = source
            .split("#[cfg(test)]")
            .next()
            .expect("expected runtime listener source before tests");
        let process_failure_section = runtime_source
            .split(".process_fetched_transaction(")
            .nth(1)
            .and_then(|tail| tail.split("None =>").next())
            .expect("expected program transaction processing section");

        assert!(
            process_failure_section.contains("failed_candidate = Some(candidate.clone());"),
            "a projection failure must retain the failing signature instead of advancing cursors past it"
        );
        assert!(
            process_failure_section.contains("break;"),
            "a projection failure must stop the candidate batch so later signatures are not checkpointed first"
        );
        assert!(
            !process_failure_section.contains("continue;"),
            "continuing after a projection failure can permanently skip the failed signature"
        );
    }

    #[test]
    fn local_program_listener_defers_global_checkpoint_until_batch_success() {
        let source = include_str!("local_program_listener.rs");
        let runtime_source = source
            .split("#[cfg(test)]")
            .next()
            .expect("expected runtime listener source before tests");
        let candidate_loop_section = runtime_source
            .split("for (idx, candidate) in candidates.iter().enumerate()")
            .nth(1)
            .and_then(|tail| {
                tail.split("if let Some(candidate) = failed_candidate")
                    .next()
            })
            .expect("expected candidate loop before failure guard");
        let success_section = runtime_source
            .split("if let Some(candidate) = failed_candidate")
            .nth(1)
            .and_then(|tail| tail.split("self.advance_program_cursors").next())
            .expect("expected batch success section before cursor advancement");

        assert!(
            !candidate_loop_section.contains("checkpoint_manager.update(candidate.slot)"),
            "global slot checkpoint must not advance per tx because a later tx in the same slot can still fail"
        );
        assert!(
            success_section.contains("self.checkpoint_manager.update(last.slot).await?"),
            "global slot checkpoint should advance only after the full candidate batch succeeds"
        );
    }

    #[test]
    fn program_cursor_rpc_failures_back_off_without_exceeding_the_cap() {
        let base = Duration::from_millis(1_500);

        assert_eq!(rpc_failure_backoff(base, 1), Duration::from_millis(1_500));
        assert_eq!(rpc_failure_backoff(base, 2), Duration::from_millis(3_000));
        assert_eq!(rpc_failure_backoff(base, 3), Duration::from_millis(6_000));
        assert_eq!(rpc_failure_backoff(base, 4), Duration::from_millis(12_000));
        assert_eq!(rpc_failure_backoff(base, 6), Duration::from_millis(30_000));
        assert_eq!(rpc_failure_backoff(base, 30), Duration::from_millis(30_000));
    }
}
