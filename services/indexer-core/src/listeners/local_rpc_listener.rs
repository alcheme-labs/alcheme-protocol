    ...
    async fn process_slot(&mut self, slot: u64) -> Result<()> {
        ...
        for (idx, signature, tx_result, tx_attempt) in fetched_results {
            ...
            match tx_result {
                ...
                Ok(None) => {
                    warn!(
                        "Transaction {} at slot {} was unavailable after {} attempt(s)",
                        signature, slot, tx_attempt
                    );
                    failed_txs_in_slot = failed_txs_in_slot.saturating_add(1);
                }
                Err(error) => {
            ...
            if failed_txs_in_slot > 0 && failed_txs_in_slot >= self.max_failed_txs_per_slot {
                return Err(anyhow!(
        ...
        if failed_txs_in_slot > 0 {
            return Err(anyhow!(
                "slot {} had {} failed tracked tx(s); retrying before marking the slot completed",
                slot,
                failed_txs_in_slot
            ));
        }
...
impl RpcJsonClient {
    ...
    pub(crate) async fn get_block_signatures(&self, slot: u64) -> Result<Option<Vec<String>>> {
        let response = self
            .call_raw(
                "getBlock",
                json!([
    ...
    async fn call_raw_inner(
        ...
        method: &str,
        params: Value,
    ) -> Result<(RpcResponse<Value>, usize)> {
        let payload = json!({
            "jsonrpc": "2.0",
            "id": 1,
            "method": method,
...
pub(crate) fn extract_primary_account_from_single_instruction_tx(
    transaction: &RpcTransactionResult,
) -> Option<String> {
...
mod tests {
    ...
    #[test]
    fn local_rpc_listener_retries_slot_when_any_tracked_tx_fails() {
        let source = include_str!("local_rpc_listener.rs");
        let runtime_source = source
            .split("#[cfg(test)]")
            .next()
            .expect("expected runtime listener source before tests");
        let failed_tx_section = runtime_source
            .split("let mut failed_txs_in_slot")
            .nth(1)
            .and_then(|tail| {
                tail.split("self.runtime_state_store.mark_slot_completed")
                    .next()
            })
            .expect("expected failed tx handling section before slot completion");

        assert!(
            failed_tx_section.contains("if failed_txs_in_slot > 0"),
            "a slot with any failed tracked tx must be retried instead of being marked completed"
        );
    }
}
