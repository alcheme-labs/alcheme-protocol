...
pub struct ContentInteractionEventContext<'a> {
...
impl DbWriter {
    ...
    pub async fn upsert_user_from_identity_snapshot(
        ...
        debug!(
            "Upserted user from identity snapshot: {} ({})",
            handle, pubkey
        );
        self.invalidate(&format!("user:{}", handle)).await;
        Ok(())
    ...
    pub async fn record_interaction(
        ...
        timestamp: i64,
        context: ContentInteractionEventContext<'_>,
    ) -> Result<()> {
        let normalized_interaction_type = normalize_content_interaction_type(interaction_type);
        // 根据交互类型更新统计
        match normalized_interaction_type {
            "Like" => {
            ...
            "Share" | "Comment" => {
                self.record_counted_content_interaction(
                    content_id,
                    actor_pubkey,
    ...
    async fn record_counted_content_interaction(
        &self,
        content_id: &str,
        actor_pubkey: &str,
        interaction_type: &str,
        ...
        let counter_update_sql = match interaction_type {
            "Share" => {
                "UPDATE posts SET shares_count = shares_count + 1, updated_at = NOW() WHERE id = $1"
            }
            "Comment" => {
        ...
        let event_slot = context.slot.and_then(|value| i64::try_from(value).ok());
        let mut tx = self.pool.begin().await?;

        let post = sqlx::query_as::<_, (i32, String)>(
            "SELECT id, content_id FROM posts WHERE content_id = $1 OR on_chain_address = $1",
        )
        .bind(content_id)
        .fetch_optional(&mut *tx)
        .await?;

        let Some((post_id, canonical_content_id)) = post else {
            warn!(
                "Post {} not found while recording {} interaction",
                content_id, interaction_type
            );
            return Ok(());
        };
        let event_key = build_content_interaction_event_key(
    ...
    pub async fn reconcile_content_post_snapshot(
        ...
        let mut tx = self.pool.begin().await?;
        let content_lookup_candidates =
            build_content_post_lookup_candidates(content_id, legacy_content_id);
        ...
        if let Some(legacy_content_id) = legacy_content_id {
            self.invalidate(&format!("post:{}", legacy_content_id))
                .await;
        }
    ...
    pub async fn upsert_circle_member(
        ...
        let user_id = self.get_or_create_user_id(member_pubkey, timestamp).await?;
    ...
    pub async fn archive_circle(
        ...
        let slot = i64::try_from(slot.context("Circle lifecycle projection slot required")?)
            .context("Circle lifecycle projection slot overflow")?;
        sqlx::query(
            r#"
            UPDATE circles
            SET lifecycle_status = 'Archived'::"CircleLifecycleStatus",
                ...
                last_synced_slot = $5,
                updated_at = NOW()
            WHERE id = $1 AND last_synced_slot <= $5
            "#,
        )
        .bind(circle_id)
        .bind(timestamp as f64)
    ...
    pub async fn restore_circle(
        ...
        sqlx::query(
            ...
            SET lifecycle_status = 'Active'::"CircleLifecycleStatus",
                ...
                last_synced_slot = $2,
                updated_at = NOW()
            WHERE id = $1 AND last_synced_slot <= $2
            "#,
        )
        .bind(circle_id)
        .bind(slot)
    ...
    fn log_best_effort_side_effect(label: &str, subject: &str, result: Result<()>) {
        if let Err(error) = result {
            warn!(
                "Skipping non-critical {} side effect for {}: {:?}",
                label, subject, error
            );
        }
    }

    async fn write_default_knowledge_relationship_assignment_best_effort(
        &self,
        knowledge_id: &str,
    ) {
        ...
        let result = sqlx::query(
            ...
            SELECT $1, l.key, '[]'::jsonb, 'system_default', NOW(), NOW()
            FROM knowledge_relationship_labels l
            WHERE l.key = 'original'
        ...
        match result {
            Ok(done) if done.rows_affected() == 0 => {
                debug!(
                    "Default knowledge relationship assignment not inserted for {}; assignment already exists or original label is missing",
                    knowledge_id
                );
            }
            Ok(_) => {}
            Err(error) => {
                warn!(
                    "Skipping default knowledge relationship assignment for {}: {:?}",
                    knowledge_id, error
                );
            }
        }
    ...
    async fn create_knowledge_submitted_notifications_best_effort(
        &self,
        author_id: i32,
        knowledge_id: &str,
        title: &str,
        ...
        Self::log_best_effort_side_effect(
            "knowledge_submitted notifications",
            knowledge_id,
            result,
        );
    }

    async fn create_reference_side_effects_best_effort(
        &self,
        canonical_source_knowledge_id: &str,
        canonical_target_knowledge_id: &str,
    ) {
        let notification_result: Result<()> = async {
        ...
        .await;
        Self::log_best_effort_side_effect(
            "reference citation notification",
            canonical_target_knowledge_id,
            notification_result,
        );

        let totem_result: Result<()> = async {
    ...
    pub async fn upsert_knowledge(
        ...
        self.write_default_knowledge_relationship_assignment_best_effort(knowledge_id)
            .await;

        let version_event_result = self
            .record_knowledge_version_event(
                knowledge_id,
                "knowledge_submitted",
                version,
                Some(author_pubkey),
            ...
            .await;
        Self::log_best_effort_side_effect(
            "knowledge_submitted version event",
            knowledge_id,
            version_event_result,
        );

        self.create_knowledge_submitted_notifications_best_effort(
            author_id,
            knowledge_id,
            title,
            circle_id,
        ...
        let totem_result = self.evaluate_totem(author_id).await;
        Self::log_best_effort_side_effect(
            "knowledge_submitted totem evaluation",
            knowledge_id,
            totem_result,
        );
    ...
    pub async fn handle_reference_added(
        ...
        self.create_reference_side_effects_best_effort(
            &canonical_source_knowledge_id,
            &canonical_target_knowledge_id,
        )
        .await;
    ...
    pub async fn upsert_knowledge_binding(
        ...
        let version_event_result = self
            ...
            .await;
        Self::log_best_effort_side_effect(
            "contributor_proof_bound version event",
            knowledge_id,
            version_event_result,
        );
    ...
    pub async fn update_knowledge_contributors(
        ...
        let version_event_result = self
            ...
            .await;
        Self::log_best_effort_side_effect(
            "contributors_updated version event",
            knowledge_id,
            version_event_result,
        );
...
fn normalize_content_interaction_type(interaction_type: &str) -> &str {
    interaction_type.trim().trim_matches('"')
}

fn build_content_interaction_event_key(
    content_id: &str,
    actor_pubkey: &str,
    interaction_type: &str,
    timestamp: i64,
    ...
    if let Some(signature) = tx_signature {
        ...
        return format!("tx:{}:{}", signature, index);
    }

    format!(
        "legacy:{}:{}:{}:{}",
        content_id, actor_pubkey, interaction_type, timestamp
    )
}
...
fn derive_legacy_numeric_content_post_on_chain_address(
    ...
    let (content_post_pda, _) = Pubkey::find_program_address(
        &[
            b"content_post",
            author.as_ref(),
            &numeric_content_id.to_le_bytes(),
        ],
        content_program_id,
    );
...
mod tests {
    ...
    #[test]
    fn share_and_comment_interactions_are_event_deduped() {
        let source = include_str!("db_writer.rs");
        let record_interaction_section = source
            .split("pub async fn record_interaction")
            .nth(1)
            .and_then(|tail| tail.split("/// 审核内容").next())
            .expect("expected record_interaction implementation section");

        assert!(
            record_interaction_section.contains("content_interaction_events"),
            "Share/Comment projections need an event-level dedupe table before incrementing counters"
        );
        assert!(
            record_interaction_section.contains("ON CONFLICT (event_key) DO NOTHING"),
            "Share/Comment projections must be replay-safe for the same chain event"
        );
        assert!(
            record_interaction_section.contains("SELECT id, content_id FROM posts"),
            "Share/Comment projections must resolve the canonical posts.content_id before writing the event table"
        );
        assert!(
            record_interaction_section.contains(".bind(&canonical_content_id)"),
            "content_interaction_events.content_id must use the canonical posts.content_id, not a raw on_chain_address lookup key"
        );
        assert!(
            !record_interaction_section.contains(
                "UPDATE posts SET shares_count = shares_count + 1 WHERE content_id = $1 OR on_chain_address = $1"
            ),
            "Share projections must not directly increment counters without an event insert guard"
        );
        assert!(
            !record_interaction_section.contains(
                "UPDATE posts SET comments_count = comments_count + 1 WHERE content_id = $1 OR on_chain_address = $1"
            ),
            "Comment projections must not directly increment counters without an event insert guard"
        );
    }

    #[test]
    fn content_interaction_event_key_uses_transaction_signature_and_event_index() {
        let base = ContentInteractionEventContext {
            tx_signature: Some("tx_sig"),
            event_index: Some(2),
            slot: Some(42),
        };
        let same = build_content_interaction_event_key("content", "actor", "Share", 1000, base);
        let different_index = build_content_interaction_event_key(
            "content",
            "actor",
            "Share",
            1000,
            ContentInteractionEventContext {
                event_index: Some(3),
                ..base
            },
        );

        assert_eq!(same, "tx:tx_sig:2");
        assert_ne!(same, different_index);
    }
    ...
    #[test]
    fn upsert_knowledge_keeps_core_projection_fatal_but_side_effects_best_effort() {
        let source = include_str!("db_writer.rs");
        let upsert_knowledge_section = source
            .split("pub async fn upsert_knowledge(")
            .nth(1)
            .and_then(|tail| {
                tail.split("pub async fn reconcile_knowledge_snapshot(")
                    .next()
            })
            .expect("expected to isolate upsert_knowledge implementation");

        assert!(
            upsert_knowledge_section.contains(".context(\"Failed to upsert knowledge\")?"),
            "core knowledge projection must remain fatal"
        );
        assert!(
            upsert_knowledge_section
                .contains("write_default_knowledge_relationship_assignment_best_effort"),
            "default assignment should be projected through a non-blocking helper"
        );
        assert!(
            upsert_knowledge_section.contains("knowledge_submitted version event"),
            "knowledge version history should be logged as a best-effort side effect"
        );
        assert!(
            upsert_knowledge_section
                .contains("create_knowledge_submitted_notifications_best_effort"),
            "notification writes should be routed through one non-blocking helper"
        );
        assert!(
            upsert_knowledge_section.contains("knowledge_submitted totem evaluation"),
            "totem evaluation should not block core knowledge projection"
        );
        assert!(
            !upsert_knowledge_section
                .contains("Failed to write default knowledge relationship assignment"),
            "missing catalog seed must not abort knowledge indexing"
        );
        assert!(
            !upsert_knowledge_section.contains("Failed to create crystal notification"),
            "notification side effects must not abort knowledge indexing"
        );
        assert!(
            !upsert_knowledge_section.contains("self.evaluate_totem(author_id).await?"),
            "totem side effects must not abort knowledge indexing"
        );
    }

    #[test]
    fn default_assignment_helper_reads_system_catalog_without_overwriting_future_labels() {
        let source = include_str!("db_writer.rs");
        let helper_section = source
            .split("async fn write_default_knowledge_relationship_assignment_best_effort")
            .nth(1)
            .and_then(|tail| {
                tail.split("async fn create_knowledge_submitted_notifications_best_effort")
                    .next()
            })
            .expect("expected to isolate default assignment helper");

        assert!(
            helper_section.contains("INSERT INTO knowledge_relationship_assignments"),
            "default assignment helper must write the projection table"
        );
        assert!(
            helper_section.contains("FROM knowledge_relationship_labels l"),
            "default assignment must use the local system catalog as its source of truth"
        );
        assert!(
            helper_section.contains("WHERE l.key = 'original'"),
            "new knowledge should default to the original relationship label"
        );
        assert!(
            helper_section.contains("'system_default'"),
            "new knowledge should record system_default assignment provenance"
        );
        assert!(
            helper_section.contains("'[]'::jsonb"),
            "default assignment should not infer source knowledge relationships"
        );
        assert!(
            helper_section.contains("ON CONFLICT (knowledge_id) DO NOTHING"),
            "default assignment must not overwrite future AI/admin labels"
        );
    }

    #[test]
    fn upsert_knowledge_binding_keeps_projection_fatal_but_history_best_effort() {
        let source = include_str!("db_writer.rs");
        let section = source
            .split("pub async fn upsert_knowledge_binding(")
            .nth(1)
            .and_then(|tail| {
                tail.split("pub async fn update_knowledge_contributors(")
                    .next()
            })
            .expect("expected to isolate upsert_knowledge_binding implementation");

        assert!(
            section.contains(".context(\"Failed to upsert knowledge binding projection\")?"),
            "binding projection must remain fatal"
        );
        assert!(
            section.contains("let version_event_result = self")
                && section.contains(".record_knowledge_version_event("),
            "binding history should be captured as a non-blocking result"
        );
        assert!(
            section.contains("contributor_proof_bound version event"),
            "binding history failures should be logged with a specific side-effect label"
        );
    }

    #[test]
    fn handle_reference_added_keeps_projection_fatal_but_side_effects_best_effort() {
        let source = include_str!("db_writer.rs");
        let section = source
            .split("pub async fn handle_reference_added(")
            .nth(1)
            .and_then(|tail| tail.split("async fn resolve_canonical_knowledge_id").next())
            .expect("expected to isolate handle_reference_added implementation");

        assert!(
            section.contains(".context(\"Failed to upsert knowledge reference\")?"),
            "reference projection insert must remain fatal"
        );
        assert!(
            section.contains(".context(\"Failed to update citation count\")?"),
            "derived citation count must remain fatal after the reference row is accepted"
        );
        assert!(
            section.contains("create_reference_side_effects_best_effort"),
            "reference notifications and totem updates should share one non-blocking helper"
        );
        assert!(
            !section.contains("Failed to create citation notification"),
            "citation notification failure must not abort reference projection"
        );
        assert!(
            !section.contains("self.evaluate_totem(author_id).await?"),
            "reference totem side effects must not abort reference projection"
        );
    }

    #[test]
    fn update_knowledge_contributors_keeps_projection_fatal_but_history_best_effort() {
        let source = include_str!("db_writer.rs");
        let section = source
            .split("pub async fn update_knowledge_contributors(")
            .nth(1)
            .and_then(|tail| {
                tail.split("async fn record_knowledge_version_event(")
                    .next()
            })
            .expect("expected to isolate update_knowledge_contributors implementation");

        assert!(
            section.contains(".context(\"Failed to update knowledge contributors\")?"),
            "contributor projection update must remain fatal"
        );
        assert!(
            section.contains("let version_event_result = self")
                && section.contains(".record_knowledge_version_event("),
            "contributor history should be captured as a non-blocking result"
        );
        assert!(
            section.contains("contributors_updated version event"),
            "contributor history failures should be logged with a specific side-effect label"
        );
    }
    ...
    #[test]
    fn circle_lifecycle_projection_is_slot_monotonic_and_activation_independent() {
        let source = include_str!("db_writer.rs");
        let archive = source
            .split("pub async fn archive_circle(")
            .nth(1)
            .and_then(|tail| tail.split("pub async fn restore_circle(").next())
            .expect("expected archive_circle implementation");
        let restore = source
            .split("pub async fn restore_circle(")
            .nth(1)
            .and_then(|tail| tail.split("pub async fn update_circle_flags(").next())
            .expect("expected restore_circle implementation");

        for section in [archive, restore] {
            assert!(section.contains("last_synced_slot ="));
            assert!(section.contains("last_synced_slot <="));
            assert!(section.contains("Circle lifecycle projection slot required"));
            assert!(!section.contains("governance_activation_states"));
        }
    }
}
