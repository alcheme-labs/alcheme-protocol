...
impl CheckpointManager {
    ...
    pub async fn update(&self, slot: u64) -> Result<()> {
        self.update_checkpoint(slot, None).await
    }

    pub async fn update_transaction(&self, slot: u64, signature: Option<&str>) -> Result<()> {
        self.update_checkpoint(slot, signature).await
    }

    async fn update_checkpoint(&self, slot: u64, signature: Option<&str>) -> Result<()> {
        sqlx::query(checkpoint_upsert_sql())
        ...
        debug!(
            "Updated checkpoint to slot: {} signature: {:?}",
            slot, signature
        );
        Ok(())
    }
    ...
    pub async fn pending_failed_slot_stats(&self) -> Result<PendingFailedSlotStats> {
        ...
        Ok(PendingFailedSlotStats {
            ...
            oldest_age_seconds: oldest_age_seconds.and_then(|age| {
                if age >= 0 {
                    Some(age as u64)
                } else {
                    None
    ...
    pub async fn prune_resolved_failed_slots(
        &self,
...
fn checkpoint_upsert_sql() -> &'static str {
    r#"
    ...
    ON CONFLICT (program_id) DO UPDATE SET
      last_processed_signature = CASE
        WHEN EXCLUDED.last_processed_slot > sync_checkpoints.last_processed_slot
          THEN EXCLUDED.last_processed_signature
        WHEN EXCLUDED.last_processed_slot = sync_checkpoints.last_processed_slot
          AND EXCLUDED.last_processed_signature IS NOT NULL
          THEN EXCLUDED.last_processed_signature
        ELSE sync_checkpoints.last_processed_signature
      END,
...
mod tests {
    ...
    #[test]
    fn projected_checkpoint_sql_tracks_signature_without_regressing_slot() {
        let sql = checkpoint_upsert_sql();
        assert!(sql.contains("last_processed_signature"));
        assert!(sql.contains("EXCLUDED.last_processed_slot > sync_checkpoints.last_processed_slot"));
        assert!(sql.contains("EXCLUDED.last_processed_slot = sync_checkpoints.last_processed_slot"));
        assert!(sql.contains("EXCLUDED.last_processed_signature IS NOT NULL"));
        assert!(sql.contains("ELSE sync_checkpoints.last_processed_signature"));
    }
}
