...
# ExternalApp V3B bounded economics chain anchoring. Disabled by default so
# local app-store and registry flows do not accidentally create custody flows.
EXTERNAL_APP_ECONOMICS_MODE="${EXTERNAL_APP_ECONOMICS_MODE:-disabled}"
EXTERNAL_APP_ECONOMICS_AUTHORITY_KEYPAIR_PATH="${EXTERNAL_APP_ECONOMICS_AUTHORITY_KEYPAIR_PATH:-}"
EXTERNAL_APP_ECONOMICS_AUTHORITY_SIGNER_URL="${EXTERNAL_APP_ECONOMICS_AUTHORITY_SIGNER_URL:-}"
EXTERNAL_APP_ECONOMICS_AUTHORITY_SIGNER_TOKEN="${EXTERNAL_APP_ECONOMICS_AUTHORITY_SIGNER_TOKEN:-}"
EXTERNAL_APP_ECONOMICS_IDL_PATH="${EXTERNAL_APP_ECONOMICS_IDL_PATH:-}"
EXTERNAL_APP_SETTLEMENT_ASSET_MODE="${EXTERNAL_APP_SETTLEMENT_ASSET_MODE:-disabled}"
EXTERNAL_APP_SETTLEMENT_TEST_MINT="${EXTERNAL_APP_SETTLEMENT_TEST_MINT:-}"
EXTERNAL_APP_SETTLEMENT_TEST_MINT_AUTHORITY="${EXTERNAL_APP_SETTLEMENT_TEST_MINT_AUTHORITY:-}"
...
load_frontend_program_ids() {
  ...
  EXTERNAL_APP_ECONOMICS_PROGRAM_ID="${NEXT_PUBLIC_EXTERNAL_APP_ECONOMICS_PROGRAM_ID:-${EXTERNAL_APP_ECONOMICS_PROGRAM_ID:-}}"
  if [[ -z "$EXTERNAL_APP_ECONOMICS_PROGRAM_ID" && -f "$ROOT_DIR/target/deploy/external_app_economics-keypair.json" ]]; then
    EXTERNAL_APP_ECONOMICS_PROGRAM_ID="$(solana address -k "$ROOT_DIR/target/deploy/external_app_economics-keypair.json" 2>/dev/null || true)"
  fi
...
check_core_programs() {
  ...
  if ! program_is_deployed "$EXTERNAL_APP_ECONOMICS_PROGRAM_ID"; then missing+=("external_app_economics:$EXTERNAL_APP_ECONOMICS_PROGRAM_ID"); fi
  ...
  cat > "$ROOT_DIR/sdk/localnet-config.json" <<EOF
{
  ...
  "programIds": {
    ...
    "externalAppEconomics": "$EXTERNAL_APP_ECONOMICS_PROGRAM_ID"
  }
}
EOF
}
...
ExternalApp V3B:   program=$EXTERNAL_APP_ECONOMICS_PROGRAM_ID mode=$EXTERNAL_APP_ECONOMICS_MODE settlement=$EXTERNAL_APP_SETTLEMENT_ASSET_MODE
