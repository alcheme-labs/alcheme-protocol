...
CRYSTAL_MINT_ADAPTER="${CRYSTAL_MINT_ADAPTER:-disabled}"
...
check_required_pdas() {
  ...
  set +e
  node - "$ROOT_DIR" "$RPC_URL" "$IDENTITY_PROGRAM_ID" "$CONTENT_PROGRAM_ID" "$ACCESS_PROGRAM_ID" "$EVENT_PROGRAM_ID" "$FACTORY_PROGRAM_ID" "$MESSAGING_PROGRAM_ID" "$CIRCLES_PROGRAM_ID" <<'NODE'
const path = require('path');

const [,, rootDir, rpc, identityId, contentId, accessId, eventId, factoryId, messagingId, circlesId] = process.argv;
const candidates = [
  path.join(rootDir, 'node_modules', '@solana', 'web3.js'),
  path.join(rootDir, 'frontend', 'node_modules', '@solana', 'web3.js'),
  path.join(rootDir, 'services', 'query-api', 'node_modules', '@solana', 'web3.js'),
...
async function main() {
  ...
  const [identityRegistry, contentManager, accessController, circleManager, eventEmitter, registryFactory, messagingManager] = await Promise.all([
  ...
  if (!identityRegistry || !contentManager || !accessController || !circleManager || !eventEmitter || !registryFactory || !messagingManager) {
    process.exit(1);
  }
}
