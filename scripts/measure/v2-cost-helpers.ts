...
export function defaultLocalTestWalletPath(): string {
  const configured = process.env.ALCHEME_LOCAL_TEST_WALLET || ".alcheme-local/test-wallet.json";
...
export function loadDefaultWallet(): Keypair {
  const walletPath = defaultLocalTestWalletPath();
  const raw = fs.readFileSync(walletPath, "utf8");
  const secret = Uint8Array.from(JSON.parse(raw));
  return Keypair.fromSecretKey(secret);
}

export function measurementRpcUrl(): string {
  return process.env.ANCHOR_PROVIDER_URL || LOCAL_RPC_URL;
}
...
export async function createMeasurementContext() {
  const connection = new Connection(measurementRpcUrl(), "confirmed");
